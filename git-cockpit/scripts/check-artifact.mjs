...
const required = [
  ...
  [/GIT_COCKPIT_PROBE/, "the fixed prerequisite probe"],
  [/GIT_OPTIONAL_LOCKS=0 git status/, "the non-locking status template"],
  [/git for-each-ref/, "the fixed branch template"],
  [/git log -n 30/, "the fixed log template"],
  [/execute_bash_command/, "the controlled Agent Server command endpoint"],
  [/Planned write operations/, "the visually distinct roadmap area"],
  [/Not available yet/, "the disabled-control explanation"],
  [/A glass cockpit, not a terminal/, "the nested safety route"],
];
...
console.log(`Artifact OK: one self-contained Blob-compatible ESM file (${info.size.toLocaleString()} bytes), with inline CSS and no sibling runtime assets.`);
