...
var e = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), t = /* @__PURE__ */ e(((e) => {
	...
	function Fc(e, t, n) {
		...
		switch (t.lanes = 0, t.tag) {
			...
			case 27: return k(t), e === null && M && (r = t.stateNode = ff(t.type, t.pendingProps, _e.current), Vi = t, Ui = !0, i = j, Zd(t.type) ? (lf = i, j = cf(r.firstChild)) : j = i), sc(e, t, t.pendingProps.children, n), _c(e, t), e === null && (t.flags |= 4194304), t.child;
			case 5: return e === null && M && ((i = r = j) && (r = tf(r, t.type, t.pendingProps, Ui), r === null ? i = !1 : (t.stateNode = r, Vi = t, j = cf(r.firstChild), Ui = !1, i = !0)), i || Gi(t)), k(t), i = t.type, a = t.pendingProps, o = e === null ? null : e.memoizedProps, r = a.children, Ud(i, a) ? r = null : o !== null && Ud(i, o) && (t.flags |= 32), t.memoizedState !== null && (i = Do(e, t, Ao, null, null, n), Qf._currentValue = i), _c(e, t), sc(e, t, r, n), t.child;
	...
	function Vc(e, t, n) {
		...
		switch (zi(t), t.tag) {
			...
			case 19:
				...
				return r.tail === null ? (V(t), null) : (e = r.tail, r.rendering = e, r.tail = e.sibling, r.renderingStartTime = Ne(), e.sibling = null, n = P.current, O(P, i ? n & 1 | 2 : n & 1), M && Ii(t, r.treeForkCount), e);
			case 22:
	...
	n(), t.exports = s();
})), l = i(), u = c(), d = 24, f = 240, p = String.raw`git_status=1
worktree_status=0
git_version=''
installer='none'
install_supported=0
...
printf 'GIT_COCKPIT_PROBE\t%s\t%s\t%s\t%s\t%s\n' "$git_status" "$worktree_status" "$installer" "$install_supported" "$version64"`, m = "GIT_OPTIONAL_LOCKS=0 git status --porcelain=v2 --branch -z --untracked-files=all", h = "git for-each-ref --sort=-committerdate --count=100 --format='R%00%(refname:short)%00%(objectname)%00%(HEAD)%00%(committerdate:iso-strict)%00' refs/heads refs/remotes", g = "git log -n 30 --format='C%x00%H%x00%P%x00%an%x00%ae%x00%aI%x00%s%x00'", _ = String.raw`printf 'GIT_COCKPIT_UNSTAGED\0'
git diff --no-ext-diff --no-color --numstat -z -- .
printf 'GIT_COCKPIT_STAGED\0'
git diff --cached --no-ext-diff --no-color --numstat -z -- .`, v = {
...
}, y = "Set up Git for Git Cockpit on this Agent Server. First inspect the operating system and available package manager. If Git is missing, explain the exact package-manager command and files it may change, obtain any needed approval, then install Git. Do not modify any repository files, Git refs, the index, remotes, credentials, or configuration. Verify with \"git --version\" and \"git rev-parse --is-inside-work-tree\" in the selected workspace, then report what changed.";
function b() {
	return new DOMException("The Agent Server request was cancelled.", "AbortError");
}
function x(e) {
	if (e?.aborted) throw b();
}
function ee(e) {
	try {
...
function te(e) {
	return typeof e != "string" || e.length === 0 || e.length > 4096 || e.startsWith("/") || /[\0-\x1f\x7f]/.test(e) || e.includes("//") ? !1 : e.split("/").every((e) => e.length > 0 && e !== "." && e !== "..");
}
function ne(e) {
	return S(e).replaceAll("+", "-").replaceAll("/", "_").replace(/=+$/g, "");
}
function re(e) {
	return ne(e);
}
...
async function ie(e, t) {
	...
	for (let e of n?.workspaces ?? []) C(e.path) && r.set(e.path, w(e.path, e.name));
	let i = (n?.workspaceParents ?? []).map((e) => e.path).filter(C).slice(0, d), a = await Promise.all(i.map(async (t) => {
		try {
	...
	x(t);
	for (let e of a) for (let t of e.items ?? []) C(t.path) && t.is_dir !== !1 && r.set(t.path, w(t.path, t.name));
	return [...r.values()].sort((e, t) => e.name.localeCompare(t.name) || e.path.localeCompare(t.path)).slice(0, f);
}
async function ae(e, t, n, r, i) {
	if (!C(t.path) || t.token !== ne(t.path)) throw Error("The selected workspace did not pass path validation. Refresh the workspace list and try again.");
	x(i);
	...
	if (o !== 0) throw Error(c || `The controlled Agent Server command failed with exit code ${o}.`);
	return s;
}
async function oe(e, t, n) {
	let r = (await ae(e, t, p, 10, n)).match(/^GIT_COCKPIT_PROBE\t([01])\t([01])\t(brew|apt|dnf|apk|none)\t([01])\t([A-Za-z0-9+/=]*)$/m);
	if (!r) throw Error("The Git prerequisite probe returned an unexpected response.");
	return {
		git: r[1] === "0",
		worktree: r[2] === "1",
		installer: r[3] === "none" ? null : r[3],
		installSupported: r[4] === "1",
		gitVersion: r[1] === "0" ? ee(r[5]) || "Git available" : null
	};
}
async function se(e, t, n, r) {
	if (n.git || !n.installer || !n.installSupported) throw Error("A supported Git install is not available for this Agent Server.");
	await ae(e, t, v[n.installer], 180, r);
}
function ce(e) {
	let t = {
		branch: null,
	...
	for (let e = 0; e < n.length; e += 1) {
		let r = n[e];
		if (r) {
			if (r.startsWith("# branch.oid ")) t.oid = r.slice(13) === "(initial)" ? null : r.slice(13);
			else if (r.startsWith("# branch.head ")) t.branch = r.slice(14) === "(detached)" ? null : r.slice(14);
			else if (r.startsWith("# branch.upstream ")) t.upstream = r.slice(18);
			else if (r.startsWith("# branch.ab ")) {
				let e = r.match(/\+(\d+) -(\d+)/);
				e && (t.ahead = Number(e[1]), t.behind = Number(e[2]));
			} else if (r.startsWith("1 ")) {
				let e = r.split(" "), n = e.slice(8).join(" ");
				te(n) && t.files.push(le(n, e[1], "tracked"));
			} else if (r.startsWith("2 ")) {
				let i = r.split(" "), a = i.slice(9).join(" "), o = n[e + 1];
				e += 1, te(a) && te(o) && t.files.push({
					...le(a, i[1], "renamed"),
					originalPath: o
				});
			} else if (r.startsWith("? ")) {
				let e = r.slice(2);
				te(e) && t.files.push(le(e, "??", "untracked"));
			}
		}
	}
	return t;
...
function T(e) {
	...
	for (let e = 0; e < t.length; e += 1) {
		if (ue(t[e]) !== "R") continue;
		let [r, i, a, o] = t.slice(e + 1, e + 5);
		r && /^[0-9a-f]{40}$/.test(i ?? "") && n.push({
			name: r,
			hash: i,
			current: a === "*",
			committedAt: o ?? ""
...
function E(e) {
	let t = e.split("\0"), n = [];
	for (let e = 0; e < t.length; e += 1) {
		if (ue(t[e]) !== "C") continue;
		let [r, i, a, o, s, c] = t.slice(e + 1, e + 7);
		/^[0-9a-f]{40}$/.test(r ?? "") && n.push({
			hash: r,
			parents: (i ?? "").split(" ").filter((e) => /^[0-9a-f]{40}$/.test(e)),
			author: a ?? "",
			email: o ?? "",
			authoredAt: s ?? "",
			subject: c ?? "",
...
function de(e) {
	let t = e.split("\0"), n = [], r = !1;
	for (let e = 0; e < t.length; e += 1) {
		let i = t[e];
		if (i === "GIT_COCKPIT_UNSTAGED") {
			r = !1;
			continue;
		}
		if (i === "GIT_COCKPIT_STAGED") {
			r = !0;
			continue;
		}
		let a = i.match(/^([^\t]+)\t([^\t]+)\t(.*)$/s);
		if (!a) continue;
		let o = a[3], s;
		o || (s = t[e + 1], o = t[e + 2], e += 2), !(!te(o) || s && !te(s)) && n.push({
			path: o,
			previousPath: s,
			additions: a[1] === "-" ? null : Number(a[1]),
			deletions: a[2] === "-" ? null : Number(a[2]),
			staged: r
		});
	}
	return n;
...
function pe(e) {
	if (!te(e)) throw Error("The requested file path did not pass validation.");
	let t = S(e);
	if (!/^[A-Za-z0-9+/=]+$/.test(t)) throw Error("The requested file path could not be safely encoded.");
	return String.raw`encoded_path='${t}'
...
async function me(e, t, n, r) {
	return ae(e, t, pe(n), 20, r);
}
function D(e) {
	if (!/^[0-9a-f]{40}$/.test(e)) throw Error("The requested commit ID did not pass validation.");
	return `git show --no-ext-diff --no-color --format=fuller --stat --summary --no-renames --max-count=1 ${e} -- | head -c 180000`;
}
async function O(e, t, n, r) {
	return ae(e, t, D(n), 20, r);
}
function he(e) {
	return e === "brew" ? "Homebrew will download Git and its required formula dependencies into the Homebrew prefix." : e === "apt" ? "APT will refresh package indexes, download the Git package and dependencies, and update the system package database." : e === "dnf" ? "DNF will download the Git package and dependencies and update the system package database." : "APK will download the Git package and dependencies and update the system package database.";
}
//#endregion
...
function _e(e) {
	...
	return `git-cockpit:preferences:v1:${encodeURIComponent(t)}`;
}
function ve(e) {
	try {
		let t = localStorage.getItem(_e(e));
		if (!t) return { ...ge };
		let n = JSON.parse(t);
		return {
			workspaceToken: typeof n.workspaceToken == "string" && /^[A-Za-z0-9_-]+$/.test(n.workspaceToken) ? n.workspaceToken : null,
			tab: n.tab === "history" || n.tab === "branches" ? n.tab : "changes"
		};
	} catch {
		return { ...ge };
	}
...
var be = /* @__PURE__ */ e(((e) => {
	...
	t.exports = be();
})))(), xe = "/extensions/git-cockpit/cockpit", Se = [
...
function Ce(e) {
	...
	if (t.length === 0 || t.length === 1 && t[0] === "cockpit") return { view: "picker" };
	if (t.length === 1 && t[0] === "about") return { view: "about" };
	if (t[0] === "workspace" && /^[A-Za-z0-9_-]+$/.test(t[1] ?? "")) {
		if (t.length === 2) return {
			view: "workspace",
			token: t[1],
			detail: null
		};
		if (t.length === 4 && t[2] === "file" && /^[A-Za-z0-9_-]+$/.test(t[3])) return {
			view: "workspace",
			token: t[1],
			detail: {
				kind: "file",
		...
		if (t.length === 4 && t[2] === "commit" && /^[0-9a-f]{40}$/.test(t[3])) return {
			view: "workspace",
			token: t[1],
			detail: {
				kind: "commit",
...
function Ee(e) {
	...
	return e.aborted ? t.abort() : e.addEventListener("abort", () => t.abort(), {
		once: !0,
		signal: t.signal
	}), t;
}
function De(e) {
	return e?.slice(0, 8) ?? "unborn";
}
function A(e) {
	let t = new Date(e);
	return Number.isNaN(t.getTime()) ? e || "Unknown date" : new Intl.DateTimeFormat(void 0, {
		dateStyle: "medium",
		timeStyle: "short"
	}).format(t);
}
function Oe(e) {
	...
	let t = [];
	return e.index !== "." && t.push(`index ${e.index}`), e.worktree !== "." && t.push(`worktree ${e.worktree}`), t.join(" · ") || "clean";
}
function ke({ route: e, navigate: t }) {
	return /* @__PURE__ */ (0, k.jsxs)("nav", {
		className: "gc-nav",
		"aria-label": "Git Cockpit sections",
		children: [/* @__PURE__ */ (0, k.jsx)("button", {
			...
			className: e.view === "about" ? "is-active" : "",
			onClick: () => t(`${xe}/about`),
			children: "Safety model"
		})]
	});
}
function Ae({ route: e, navigate: t, children: n }) {
		...
		children: [/* @__PURE__ */ (0, k.jsxs)("header", {
			...
			children: [
				/* @__PURE__ */ (0, k.jsxs)("button", {
					...
					type: "button",
					onClick: () => t(xe),
					"aria-label": "Git Cockpit home",
					children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							"aria-hidden": "true",
							children: "⌁"
						}),
						/* @__PURE__ */ (0, k.jsx)("strong", { children: "GIT" }),
						/* @__PURE__ */ (0, k.jsx)("b", { children: "/" }),
						"COCKPIT"
					]
				}),
				/* @__PURE__ */ (0, k.jsx)(ke, {
				...
				/* @__PURE__ */ (0, k.jsxs)("span", {
					className: "gc-readonly",
					children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Read-only"]
				})
			]
		}), n]
	});
...
function je() {
	return /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "gc-planned",
		"aria-labelledby": "planned-title",
		children: [
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-planned-heading",
				children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("p", {
					className: "gc-eyebrow",
					children: "Roadmap preview · unavailable"
				}), /* @__PURE__ */ (0, k.jsx)("h2", {
					id: "planned-title",
					children: "Planned write operations"
				})] }), /* @__PURE__ */ (0, k.jsx)("span", { children: "0 commands enabled" })]
			}),
			/* @__PURE__ */ (0, k.jsx)("p", {
				className: "gc-planned-intro",
				children: "These controls are intentionally disabled and non-functional in this read-only release. A later App revision must add explicit confirmation and carefully scoped safety checks before any of them can reach the Agent Server."
			}),
			/* @__PURE__ */ (0, k.jsx)("div", {
				className: "gc-planned-grid",
				children: Se.map(([e, t], n) => /* @__PURE__ */ (0, k.jsxs)("div", {
					className: "gc-planned-item",
					children: [/* @__PURE__ */ (0, k.jsxs)("button", {
						type: "button",
						disabled: !0,
						"data-testid": "planned-write-operation",
						"aria-describedby": `planned-${n}`,
						children: [e, /* @__PURE__ */ (0, k.jsx)("span", { children: "Planned" })]
					}), /* @__PURE__ */ (0, k.jsxs)("p", {
						id: `planned-${n}`,
						children: [
							t,
							" ",
							/* @__PURE__ */ (0, k.jsx)("strong", { children: "Not available yet." })
						]
					})]
				}, e))
			})
...
function Me({ workspaces: e, state: t, error: n, remembered: r, onSelect: i }) {
	let a = e.some((e) => e.token === r) ? r ?? "" : "";
		...
		children: [/* @__PURE__ */ (0, k.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, k.jsxs)("section", {
				...
				children: [
					/* @__PURE__ */ (0, k.jsx)("p", {
						className: "gc-eyebrow",
						children: "Repository instruments · Agent Server"
					}),
					/* @__PURE__ */ (0, k.jsxs)("h1", { children: [
						"See the whole worktree.",
						/* @__PURE__ */ (0, k.jsx)("br", {}),
						/* @__PURE__ */ (0, k.jsx)("em", { children: "Touch nothing." })
					] }),
					/* @__PURE__ */ (0, k.jsx)("p", { children: "Git Cockpit turns fixed, read-only Git commands into a focused view of changes, branches, history, file diffs, and commit detail." }),
					/* @__PURE__ */ (0, k.jsxs)("div", {
						className: "gc-boundary-note",
						children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "!" }), /* @__PURE__ */ (0, k.jsx)("p", { children: "Commands execute on the active Agent Server machine, inside the workspace you select. No repository data is sent to a third party." })]
					})
				]
			}), /* @__PURE__ */ (0, k.jsxs)("section", {
				className: "gc-picker-card",
				"aria-labelledby": "workspace-heading",
				children: [
					/* @__PURE__ */ (0, k.jsx)("span", {
						className: "gc-card-index",
						children: "01 / CONNECT"
					}),
					/* @__PURE__ */ (0, k.jsx)("h2", {
					...
					/* @__PURE__ */ (0, k.jsx)("p", { children: "Only directories reported by the active Agent Server are eligible." }),
					t === "loading" ? /* @__PURE__ */ (0, k.jsxs)("div", {
						className: "gc-loading",
						children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Discovering workspaces…"]
					}) : null,
					t === "error" ? /* @__PURE__ */ (0, k.jsxs)("div", {
						className: "gc-error",
						children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Workspace discovery failed" }), /* @__PURE__ */ (0, k.jsx)("span", { children: n })]
					}) : null,
					t === "ready" ? /* @__PURE__ */ (0, k.jsxs)(k.Fragment, { children: [
						/* @__PURE__ */ (0, k.jsx)("label", {
							htmlFor: "gc-workspace",
							children: "Agent Server workspace"
						}),
						/* @__PURE__ */ (0, k.jsxs)("select", {
							id: "gc-workspace",
							value: a,
							onChange: (e) => e.target.value && i(e.target.value),
							children: [/* @__PURE__ */ (0, k.jsx)("option", {
								value: "",
								children: "Choose a repository…"
							}), e.map((e) => /* @__PURE__ */ (0, k.jsxs)("option", {
								value: e.token,
								children: [
									e.name,
									" — ",
						...
						e.length === 0 ? /* @__PURE__ */ (0, k.jsx)("p", {
							className: "gc-empty",
							children: "No workspace directories were reported by this backend."
						}) : null
					] }) : null,
					/* @__PURE__ */ (0, k.jsxs)("div", {
						className: "gc-probe-list",
						children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "Next, Git Cockpit will:" }), /* @__PURE__ */ (0, k.jsxs)("ol", { children: [
							/* @__PURE__ */ (0, k.jsx)("li", { children: "Probe for Git" }),
							/* @__PURE__ */ (0, k.jsx)("li", { children: "Validate the worktree" }),
							/* @__PURE__ */ (0, k.jsx)("li", { children: "Load read-only signals" })
						] })]
					})
				]
			})]
...
function Ne({ workspace: e, status: t, state: n, error: r, acknowledged: i, setAcknowledged: a, installState: o, installError: s, onInstall: c, onRecheck: l, onCopy: u, copyNotice: d }) {
	return /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "gc-onboarding",
		children: [
			/* @__PURE__ */ (0, k.jsx)("p", {
				className: "gc-eyebrow",
				children: "Prerequisite gate"
			}),
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "Checking the flight deck." }),
			/* @__PURE__ */ (0, k.jsxs)("p", {
				className: "gc-onboarding-lede",
				children: [
					"The probe is non-mutating and runs in ",
					/* @__PURE__ */ (0, k.jsx)("code", { children: e.path }),
					" on the Agent Server."
				]
			}),
			n === "loading" ? /* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-loading gc-loading--large",
				children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Probing Git and worktree capabilities…"]
			}) : null,
			n === "error" ? /* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-error",
				children: [
					/* @__PURE__ */ (0, k.jsx)("strong", { children: "Prerequisite probe failed" }),
					/* @__PURE__ */ (0, k.jsx)("span", { children: r }),
					/* @__PURE__ */ (0, k.jsx)("button", {
						type: "button",
						onClick: l,
						children: "Recheck"
			...
			n === "ready" && t ? /* @__PURE__ */ (0, k.jsxs)("section", {
				className: "gc-gate-card",
				children: [
					/* @__PURE__ */ (0, k.jsxs)("div", {
						className: `gc-gate-status ${t.git ? "is-ready" : "is-missing"}`,
						children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "Git CLI" }), /* @__PURE__ */ (0, k.jsx)("strong", { children: t.git ? t.gitVersion : "Missing" })]
					}),
					/* @__PURE__ */ (0, k.jsxs)("div", {
						className: `gc-gate-status ${t.worktree ? "is-ready" : "is-missing"}`,
						children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "Selected directory" }), /* @__PURE__ */ (0, k.jsx)("strong", { children: t.worktree ? "Git worktree" : t.git ? "Not a Git worktree" : "Pending Git" })]
					}),
					t.git ? t.worktree ? /* @__PURE__ */ (0, k.jsxs)("div", {
						className: "gc-loading",
						children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Git is ready. Loading repository signals…"]
					}) : /* @__PURE__ */ (0, k.jsxs)("div", {
						className: "gc-error",
						children: [
							/* @__PURE__ */ (0, k.jsx)("strong", { children: "This workspace is not a Git worktree" }),
							/* @__PURE__ */ (0, k.jsx)("span", { children: "Choose a directory with a .git worktree, then run the probe again." }),
							/* @__PURE__ */ (0, k.jsx)("button", {
								type: "button",
								onClick: l,
								children: "Recheck"
						...
						className: "gc-install-panel",
						children: [
							/* @__PURE__ */ (0, k.jsx)("p", {
								className: "gc-eyebrow",
								children: "Setup required"
							}),
							/* @__PURE__ */ (0, k.jsx)("h2", { children: "Install Git deliberately" }),
							t.installer ? /* @__PURE__ */ (0, k.jsxs)("p", { children: [
								"Detected package manager: ",
								/* @__PURE__ */ (0, k.jsx)("strong", { children: t.installer }),
								". ",
								he(t.installer)
							] }) : /* @__PURE__ */ (0, k.jsx)("p", { children: "No supported package manager was detected. Use the fixed OpenHands-agent prompt below." }),
							t.installer ? /* @__PURE__ */ (0, k.jsx)("pre", { children: v[t.installer] }) : null,
							t.installer && t.installSupported ? /* @__PURE__ */ (0, k.jsxs)(k.Fragment, { children: [/* @__PURE__ */ (0, k.jsxs)("label", {
								className: "gc-confirm",
								children: [/* @__PURE__ */ (0, k.jsx)("input", {
									type: "checkbox",
									checked: i,
									onChange: (e) => a(e.target.checked)
								}), " I understand this changes software on the Agent Server machine."]
							}), /* @__PURE__ */ (0, k.jsx)("button", {
								className: "gc-button gc-button--danger",
								type: "button",
								disabled: !i || o === "installing",
								onClick: c,
								children: o === "installing" ? "Installing Git…" : `Install Git with ${t.installer}`
							})] }) : /* @__PURE__ */ (0, k.jsx)("p", {
								className: "gc-not-supported",
								children: "Automatic install is unavailable because the package manager is unsupported or non-interactive privileges are unavailable."
							}),
							s ? /* @__PURE__ */ (0, k.jsxs)("div", {
								className: "gc-error",
								children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Git installation failed" }), /* @__PURE__ */ (0, k.jsx)("span", { children: s })]
							}) : null,
							/* @__PURE__ */ (0, k.jsxs)("div", {
								className: "gc-setup-actions",
								children: [/* @__PURE__ */ (0, k.jsx)("button", {
									className: "gc-button",
									type: "button",
									onClick: u,
...
function Pe({ snapshot: e }) {
	let t = e.diffStats.reduce((e, t) => e + (t.additions ?? 0), 0), n = e.diffStats.reduce((e, t) => e + (t.deletions ?? 0), 0);
	return /* @__PURE__ */ (0, k.jsxs)("div", {
		className: "gc-summary-strip",
		children: [
			/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "HEAD" }), /* @__PURE__ */ (0, k.jsx)("strong", { children: De(e.status.oid) })] }),
			/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "Changed files" }), /* @__PURE__ */ (0, k.jsx)("strong", { children: e.status.files.length })] }),
			/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "Diff" }), /* @__PURE__ */ (0, k.jsxs)("strong", {
				className: "gc-delta",
				children: [
					/* @__PURE__ */ (0, k.jsxs)("i", { children: ["+", t] }),
					" ",
					/* @__PURE__ */ (0, k.jsxs)("b", { children: ["−", n] })
				]
			})] }),
			/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "Upstream" }), /* @__PURE__ */ (0, k.jsx)("strong", { children: e.status.upstream ?? "not set" })] })
		]
	});
}
function Fe({ snapshot: e, onFile: t }) {
	let n = (t) => e.diffStats.filter((e) => e.path === t.path);
	return /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "gc-panel",
		"aria-labelledby": "changes-title",
		children: [/* @__PURE__ */ (0, k.jsxs)("div", {
			className: "gc-panel-heading",
			children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("p", {
				className: "gc-eyebrow",
			...
			})] }), /* @__PURE__ */ (0, k.jsxs)("span", { children: [e.status.files.length, " visible"] })]
		}), e.status.files.length === 0 ? /* @__PURE__ */ (0, k.jsxs)("div", {
			className: "gc-empty-state",
			children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Working tree clean" }), /* @__PURE__ */ (0, k.jsx)("p", { children: "No staged, unstaged, or untracked paths were reported." })]
		}) : /* @__PURE__ */ (0, k.jsx)("div", {
			className: "gc-file-list",
			children: e.status.files.map((e) => {
				let r = n(e), i = r.reduce((e, t) => e + (t.additions ?? 0), 0), a = r.reduce((e, t) => e + (t.deletions ?? 0), 0);
				return /* @__PURE__ */ (0, k.jsxs)("button", {
					type: "button",
					onClick: () => t(e),
					children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							className: `gc-status-code gc-status-code--${e.kind}`,
							children: e.kind === "untracked" ? "?" : `${e.index}${e.worktree}`.replaceAll(".", "·")
						}),
						/* @__PURE__ */ (0, k.jsxs)("span", {
							className: "gc-file-main",
							children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: e.path }), /* @__PURE__ */ (0, k.jsxs)("small", { children: [e.originalPath ? `renamed from ${e.originalPath} · ` : "", Oe(e)] })]
						}),
						r.length ? /* @__PURE__ */ (0, k.jsxs)("span", {
							className: "gc-file-stat",
							children: [/* @__PURE__ */ (0, k.jsxs)("i", { children: ["+", i] }), /* @__PURE__ */ (0, k.jsxs)("b", { children: ["−", a] })]
						}) : /* @__PURE__ */ (0, k.jsx)("span", {
							className: "gc-file-stat gc-file-stat--empty",
							children: e.kind === "untracked" ? "not in diff" : "binary / mode"
						}),
						/* @__PURE__ */ (0, k.jsx)("span", {
							"aria-hidden": "true",
							children: "→"
...
function Ie({ commits: e, onCommit: t }) {
	return /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "gc-panel",
		...
		children: [/* @__PURE__ */ (0, k.jsxs)("div", {
			...
			})] }), /* @__PURE__ */ (0, k.jsxs)("span", { children: [e.length, " commits"] })]
		}), e.length === 0 ? /* @__PURE__ */ (0, k.jsxs)("div", {
			className: "gc-empty-state",
			children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "No commits yet" }), /* @__PURE__ */ (0, k.jsx)("p", { children: "This may be an unborn repository." })]
		}) : /* @__PURE__ */ (0, k.jsx)("div", {
			className: "gc-commit-list",
			children: e.map((e) => /* @__PURE__ */ (0, k.jsxs)("button", {
				type: "button",
				onClick: () => t(e),
				children: [
					/* @__PURE__ */ (0, k.jsx)("span", { className: "gc-graph-dot" }),
					/* @__PURE__ */ (0, k.jsxs)("span", {
						className: "gc-commit-main",
						children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: e.subject || "(no subject)" }), /* @__PURE__ */ (0, k.jsxs)("small", { children: [
							e.author,
							" · ",
							A(e.authoredAt)
						] })]
					}),
					/* @__PURE__ */ (0, k.jsx)("code", { children: De(e.hash) }),
					/* @__PURE__ */ (0, k.jsx)("span", {
						"aria-hidden": "true",
						children: "→"
					})
		...
		children: [
			/* @__PURE__ */ (0, k.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("p", {
					...
					id: "branches-title",
					children: "Branches"
				})] }), /* @__PURE__ */ (0, k.jsxs)("span", { children: [e.branches.length, " refs"] })]
			}),
			/* @__PURE__ */ (0, k.jsx)("div", {
				className: "gc-branch-list",
				children: e.branches.map((e) => /* @__PURE__ */ (0, k.jsxs)("div", {
					className: e.current ? "is-current" : "",
					children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							className: "gc-branch-mark",
							children: e.current ? "●" : "○"
						}),
						/* @__PURE__ */ (0, k.jsx)("strong", { children: e.name }),
						/* @__PURE__ */ (0, k.jsx)("code", { children: De(e.hash) }),
						/* @__PURE__ */ (0, k.jsx)("small", { children: A(e.committedAt) })
					]
				}, `${e.name}-${e.hash}`))
			}),
			e.branches.length === 0 ? /* @__PURE__ */ (0, k.jsx)("div", {
				className: "gc-empty-state",
				children: /* @__PURE__ */ (0, k.jsx)("strong", { children: "No branches reported" })
			}) : null
		]
	});
}
function Re({ route: e, snapshot: t, state: n, content: r, error: i, onBack: a }) {
	let o = e.detail?.kind === "file" ? t.status.files.find((t) => re(t.path) === e.detail?.value) : null, s = e.detail?.kind === "commit" ? t.commits.find((t) => t.hash === e.detail?.value) : null, c = o?.path ?? s?.subject ?? "Detail unavailable";
	return /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "gc-detail",
		children: [
			/* @__PURE__ */ (0, k.jsx)("button", {
			...
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-detail-heading",
				children: [
					/* @__PURE__ */ (0, k.jsx)("p", {
						className: "gc-eyebrow",
						children: o ? "Individual file diff" : "Commit detail"
					}),
					/* @__PURE__ */ (0, k.jsx)("h2", { children: c }),
					s ? /* @__PURE__ */ (0, k.jsxs)("p", { children: [
						s.author,
						" <",
						s.email,
						"> · ",
						A(s.authoredAt),
						" · ",
						/* @__PURE__ */ (0, k.jsx)("code", { children: s.hash })
					] }) : null
				]
			}),
			n === "loading" ? /* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-loading",
				children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Loading fixed read-only detail…"]
			}) : null,
			n === "error" ? /* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-error",
				children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Could not load detail" }), /* @__PURE__ */ (0, k.jsx)("span", { children: i })]
			}) : null,
			n === "ready" ? /* @__PURE__ */ (0, k.jsx)("pre", {
				className: "gc-diff",
				"data-testid": "detail-output",
				children: r || "No textual diff or detail was returned."
			}) : null
		]
	});
}
function ze({ workspace: e, snapshot: t, tab: n, setTab: r, route: i, navigate: a, onRefresh: o, refreshState: s, detailState: c, detailContent: l, detailError: u }) {
	let d = `${xe}/workspace/${e.token}`;
	return i.detail ? /* @__PURE__ */ (0, k.jsx)("main", {
		className: "gc-dashboard",
		children: /* @__PURE__ */ (0, k.jsx)(Re, {
			route: i,
			snapshot: t,
			state: c,
			content: l,
		...
		children: [
			/* @__PURE__ */ (0, k.jsxs)("section", {
				className: "gc-repo-heading",
				children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [
					/* @__PURE__ */ (0, k.jsx)("p", {
						className: "gc-eyebrow",
						children: "Connected worktree"
					}),
					/* @__PURE__ */ (0, k.jsx)("h1", { children: e.name }),
					/* @__PURE__ */ (0, k.jsx)("p", { children: /* @__PURE__ */ (0, k.jsx)("code", { children: e.path }) })
				] }), /* @__PURE__ */ (0, k.jsxs)("div", {
					className: "gc-repo-actions",
					children: [/* @__PURE__ */ (0, k.jsxs)("span", {
						className: "gc-branch-chip",
						children: ["⑂ ", t.status.branch ?? "detached HEAD"]
					}), /* @__PURE__ */ (0, k.jsx)("button", {
						className: "gc-button gc-button--quiet",
						type: "button",
						disabled: s === "loading",
						onClick: o,
						children: s === "loading" ? "Refreshing…" : "Refresh signals"
					})]
				})]
			}),
			/* @__PURE__ */ (0, k.jsx)(Pe, { snapshot: t }),
			/* @__PURE__ */ (0, k.jsx)("div", {
				className: "gc-tab-bar",
				role: "tablist",
				...
				children: [
					...
					role: "tab",
					"aria-selected": n === e,
					className: n === e ? "is-active" : "",
					type: "button",
					onClick: () => r(e),
					children: e
				}, e))
			}),
			n === "changes" ? /* @__PURE__ */ (0, k.jsx)(Fe, {
				snapshot: t,
				onFile: (e) => a(`${d}/file/${re(e.path)}`)
			}) : null,
			n === "history" ? /* @__PURE__ */ (0, k.jsx)(Ie, {
				commits: t.commits,
				onCommit: (e) => a(`${d}/commit/${e.hash}`)
			}) : null,
			n === "branches" ? /* @__PURE__ */ (0, k.jsx)(Le, { snapshot: t }) : null,
			/* @__PURE__ */ (0, k.jsx)(je, {})
		]
	});
}
function Be({ navigate: e }) {
	return /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "gc-about",
		children: [
			/* @__PURE__ */ (0, k.jsx)("p", {
			...
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "A glass cockpit, not a terminal." }),
			/* @__PURE__ */ (0, k.jsx)("p", {
				className: "gc-about-lede",
				children: "Git Cockpit exposes a narrow instrument panel over the active Agent Server. Every active repository operation maps to a reviewed, read-only Git command template."
			}),
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "gc-safety-grid",
				children: [
					/* @__PURE__ */ (0, k.jsxs)("article", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", { children: "01" }),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: "Controlled commands" }),
						/* @__PURE__ */ (0, k.jsxs)("p", { children: [
							"No arbitrary command field exists. Workspaces stay in the structured ",
							/* @__PURE__ */ (0, k.jsx)("code", { children: "cwd" }),
							" field; file paths are validated, base64 encoded, decoded into a quoted variable, and passed after ",
							/* @__PURE__ */ (0, k.jsx)("code", { children: "--" }),
							"; commit IDs must be full hexadecimal hashes already returned by the log."
						] })
					] }),
					/* @__PURE__ */ (0, k.jsxs)("article", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", { children: "02" }),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: "Machine boundary" }),
						/* @__PURE__ */ (0, k.jsx)("p", { children: "Commands execute on the Agent Server machine. Only harmless view preferences are stored in the browser, scoped to that backend. The App creates no repository files, service, or backend data directory." })
					] }),
					/* @__PURE__ */ (0, k.jsxs)("article", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", { children: "03" }),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: "Future writes" }),
						/* @__PURE__ */ (0, k.jsx)("p", { children: "Write-capable controls are visual roadmap placeholders only. A later revision must add command-specific validation, previews, recoverability, and explicit confirmation." })
					] })
				]
			}),
			/* @__PURE__ */ (0, k.jsx)("button", {
...
function Ve({ host: e, path: t, navigate: n, signal: r }) {
	let i = (0, l.useMemo)(() => Ce(t), [t]), [a, o] = (0, l.useState)(() => ve(e.backend)), [s, c] = (0, l.useState)([]), [u, d] = (0, l.useState)("loading"), [f, p] = (0, l.useState)(""), [m, h] = (0, l.useState)(null), [g, _] = (0, l.useState)("loading"), [v, b] = (0, l.useState)(""), [x, ee] = (0, l.useState)(null), [S, C] = (0, l.useState)("idle"), [te, ne] = (0, l.useState)(""), [w, ae] = (0, l.useState)(0), [ce, le] = (0, l.useState)(!1), [ue, T] = (0, l.useState)("idle"), [E, de] = (0, l.useState)(""), [pe, D] = (0, l.useState)(""), [he, ge] = (0, l.useState)("idle"), [_e, be] = (0, l.useState)(""), [Se, De] = (0, l.useState)(""), A = i.view === "workspace" ? s.find((e) => e.token === i.token) ?? null : null, Oe = (0, l.useCallback)((t) => {
		o((n) => {
			let r = {
				...n,
				...t
			};
			return ye(e.backend, r), r;
		});
	}, [e.backend]);
	(0, l.useEffect)(() => {
		let t = Ee(r);
		return d("loading"), ie(e, t.signal).then((e) => {
			c(e), d("ready");
		}).catch((e) => {
			Te(e) || (p(we(e)), d("error"));
		}), () => t.abort();
		...
		if (!A) return;
		let t = Ee(r);
		return h(null), b(""), _("loading"), ee(null), C("idle"), oe(e, A, t.signal).then((e) => {
			h(e), _("ready");
		}).catch((e) => {
			Te(e) || (b(we(e)), _("error"));
		}), () => t.abort();
		...
		if (!A || !m?.git || !m.worktree) return;
		let t = Ee(r);
		return C("loading"), ne(""), ee(null), fe(e, A, t.signal).then((e) => {
			ee(e), C("ready");
		}).catch((e) => {
			Te(e) || (ne(we(e)), C("error"));
		}), () => t.abort();
	}, [
		e,
		m,
		...
		if (i.view !== "workspace" || !i.detail || !A || !x) {
			ge("idle"), be("");
			return;
		}
		let t = Ee(r);
		return (async () => {
			ge("loading"), be(""), De("");
			try {
				if (i.detail?.kind === "file") {
					let n = x.status.files.find((e) => re(e.path) === i.detail?.value);
					if (!n) throw Error("That changed file is not present in the current repository status.");
					n.kind === "untracked" ? be("Untracked files have no Git diff until they are staged. Staging is not available in this read-only release.") : be(await me(e, A, n.path, t.signal));
				} else {
					let n = x.commits.find((e) => e.hash === i.detail?.value);
					if (!n) throw Error("That commit is not present in the bounded recent history.");
					be(await O(e, A, n.hash, t.signal));
				}
				ge("ready");
			} catch (e) {
				Te(e) || (De(we(e)), ge("error"));
			}
		})(), () => t.abort();
	}, [
		e,
	...
	let ke = (0, l.useCallback)(async () => {
		if (!A || !m || !ce) return;
		let t = Ee(r);
		T("installing"), de("");
		try {
			await se(e, A, m, t.signal), le(!1), ae((e) => e + 1);
		} catch (e) {
			Te(e) || de(we(e));
		} finally {
			T("idle"), t.abort();
		...
		try {
			await navigator.clipboard.writeText(y), D("Setup prompt copied.");
		} catch {
			D("Clipboard access was unavailable. Copy the prompt from the README instead.");
		}
	}, []), Pe;
	return Pe = i.view === "about" ? /* @__PURE__ */ (0, k.jsx)(Be, { navigate: n }) : i.view === "not-found" ? /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "gc-not-found",
		children: [
			/* @__PURE__ */ (0, k.jsx)("p", {
				className: "gc-eyebrow",
				children: "404 / route"
			}),
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "Instrument not found." }),
			/* @__PURE__ */ (0, k.jsx)("button", {
				className: "gc-button",
				type: "button",
				onClick: () => n(xe),
				children: "Return to cockpit"
			})
		]
	}) : i.view === "picker" ? /* @__PURE__ */ (0, k.jsx)(Me, {
		workspaces: s,
		state: u,
		error: f,
		remembered: a.workspaceToken,
		onSelect: (e) => {
			Oe({ workspaceToken: e }), n(`${xe}/workspace/${e}`);
		}
	}) : u === "ready" && !A ? /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "gc-not-found",
		children: [
			/* @__PURE__ */ (0, k.jsx)("p", {
				className: "gc-eyebrow",
			...
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "This backend did not report that workspace." }),
			/* @__PURE__ */ (0, k.jsx)("button", {
				className: "gc-button",
				type: "button",
				onClick: () => n(xe),
				children: "Choose another workspace"
			})
		]
	}) : A ? !m?.git || !m.worktree || g !== "ready" ? /* @__PURE__ */ (0, k.jsx)(Ne, {
		workspace: A,
		status: m,
		state: g,
		error: v,
		...
		onCopy: () => void je(),
		copyNotice: pe
	}) : S === "error" ? /* @__PURE__ */ (0, k.jsx)("main", {
		className: "gc-onboarding",
		children: /* @__PURE__ */ (0, k.jsxs)("div", {
			className: "gc-error",
			children: [
				/* @__PURE__ */ (0, k.jsx)("strong", { children: "Repository signals failed" }),
	...
	}) : !x || S === "loading" ? /* @__PURE__ */ (0, k.jsx)("main", {
		className: "gc-onboarding",
		children: /* @__PURE__ */ (0, k.jsxs)("div", {
			className: "gc-loading gc-loading--large",
			children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Reading status, branches, commits, and diff summary…"]
		})
	}) : /* @__PURE__ */ (0, k.jsx)(ze, {
		workspace: A,
		snapshot: x,
		...
		children: /* @__PURE__ */ (0, k.jsxs)("div", {
			className: "gc-loading gc-loading--large",
			children: [/* @__PURE__ */ (0, k.jsx)("i", {}), " Resolving selected workspace…"]
		})
	}), /* @__PURE__ */ (0, k.jsx)(Ae, {
		route: i,
		navigate: n,
...
var He = ".gc-shell,.gc-shell *{box-sizing:border-box}.gc-shell{--lightningcss-light: ;--lightningcss-dark:initial;color-scheme:dark;--ink:#f4f1e8;--muted:#9b9b92;--panel:#171816;--panel-2:#1e201d;--line:#343730;--acid:#caff4a;--cyan:#62e4dc;--orange:#ff9b55;--danger:#ff6b62;min-height:100%;color:var(--ink);background:#0d0e0c;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;line-height:1.45}.gc-shell button,.gc-shell select,.gc-shell input{font:inherit}.gc-shell button{color:inherit}.gc-header{border-bottom:1px solid var(--line);z-index:10;background:#0d0e0cf5;grid-template-columns:1fr auto 1fr;align-items:center;height:70px;padding:0 34px;display:grid;position:sticky;top:0}.gc-wordmark{cursor:pointer;letter-spacing:.06em;background:0 0;border:0;align-items:center;gap:8px;width:max-content;font-size:14px;display:flex}.gc-wordmark>span{background:var(--acid);color:#10110f;border-radius:50%;place-items:center;width:29px;height:29px;font-size:20px;font-weight:900;display:grid}.gc-wordmark strong{font-size:18px}.gc-wordmark b{color:#65675f}.gc-nav{align-items:stretch;height:100%;display:flex}.gc-nav button{color:var(--muted);cursor:pointer;background:0 0;border:0;padding:0 20px;font-size:13px;font-weight:650;position:relative}.gc-nav button:hover,.gc-nav button.is-active{color:var(--ink)}.gc-nav button.is-active:after{content:\"\";background:var(--acid);height:3px;position:absolute;bottom:-1px;left:20px;right:20px}.gc-readonly{color:var(--acid);text-transform:uppercase;letter-spacing:.08em;background:#182014;border:1px solid #35482a;border-radius:99px;justify-self:end;align-items:center;gap:8px;padding:7px 11px;font:700 11px/1 ui-monospace,SFMono-Regular,Menlo,monospace;display:flex}.gc-readonly i{background:var(--acid);width:6px;height:6px;box-shadow:0 0 10px var(--acid);border-radius:50%}.gc-picker,.gc-dashboard,.gc-onboarding,.gc-about,.gc-not-found{width:min(1220px,100% - 48px);margin:0 auto;padding:64px 0 88px}.gc-hero-grid{grid-template-columns:1.2fr .8fr;align-items:center;gap:80px;min-height:590px;display:grid;position:relative}.gc-hero-grid:before{content:\"\";pointer-events:none;background:radial-gradient(#caff4a1f,#0000 70%);height:460px;position:absolute;inset:2% 38% auto -12%}.gc-eyebrow{color:var(--acid);text-transform:uppercase;letter-spacing:.15em;margin:0 0 12px;font:750 11px/1.2 ui-monospace,SFMono-Regular,Menlo,monospace}.gc-hero-copy{position:relative}.gc-hero-copy h1,.gc-onboarding h1,.gc-about h1,.gc-not-found h1{letter-spacing:-.055em;max-width:770px;margin:0;font-size:clamp(48px,6vw,82px);line-height:.98}.gc-hero-copy h1 em{color:var(--acid);font-style:normal}.gc-hero-copy>p:not(.gc-eyebrow){color:#b7b7ae;max-width:650px;margin:28px 0;font-size:18px;line-height:1.65}.gc-boundary-note{border-left:3px solid var(--cyan);background:#62e4dc0f;align-items:flex-start;gap:14px;max-width:620px;padding:16px 18px;display:flex}.gc-boundary-note span{border:1px solid var(--cyan);width:23px;height:23px;color:var(--cyan);border-radius:50%;flex:none;place-items:center;font-weight:800;display:grid}.gc-boundary-note p{color:#bfc7c3;margin:0;font-size:13px}.gc-picker-card,.gc-gate-card{background:linear-gradient(145deg,#1d1f1b,#151613);border:1px solid #3d4038;padding:34px;position:relative;box-shadow:18px 18px #070807}.gc-card-index{color:#777a70;letter-spacing:.14em;margin-bottom:34px;font:700 10px ui-monospace,SFMono-Regular,Menlo,monospace;display:block}.gc-picker-card h2,.gc-install-panel h2{letter-spacing:-.03em;margin:0;font-size:28px}.gc-picker-card>p{color:var(--muted);font-size:13px}.gc-picker-card label{color:#c7c8c1;text-transform:uppercase;letter-spacing:.08em;margin:28px 0 8px;font:700 11px ui-monospace,SFMono-Regular,Menlo,monospace;display:block}.gc-picker-card select{width:100%;min-height:51px;color:var(--ink);background:#0e0f0d;border:1px solid #505348;border-radius:2px;outline:none;padding:0 42px 0 14px}.gc-picker-card select:focus{border-color:var(--acid);box-shadow:0 0 0 3px #caff4a1f}.gc-probe-list{border-top:1px solid var(--line);color:var(--muted);margin-top:28px;padding-top:23px;font-size:12px}.gc-probe-list>span{color:#d4d5cd;font-weight:700}.gc-probe-list ol{gap:6px;margin:12px 0 0;padding-left:24px;display:grid}.gc-empty{color:var(--muted)}.gc-loading{border:1px solid var(--line);color:#c8c9c1;background:#121310;align-items:center;gap:12px;padding:19px;font:650 12px ui-monospace,SFMono-Regular,Menlo,monospace;display:flex}.gc-loading i{border:2px solid var(--acid);border-right-color:#0000;border-radius:50%;width:10px;height:10px;animation:.8s linear infinite gc-spin}.gc-loading--large{justify-content:center;min-height:180px}@keyframes gc-spin{to{transform:rotate(360deg)}}.gc-error{color:#ffd2cf;background:#251413;border:1px solid #6f3632;gap:8px;margin:20px 0;padding:18px;display:grid}.gc-error strong{color:var(--danger)}.gc-error span{font-size:13px}.gc-error button{cursor:pointer;background:0 0;border:1px solid #8f4c47;width:max-content;margin-top:6px;padding:7px 11px}.gc-planned{background:linear-gradient(135deg,#ff9b5517,#ff6b6206);border:1px dashed #815333;margin-top:86px;padding:34px;position:relative;overflow:hidden}.gc-planned:before{content:\"PLANNED / DISABLED\";text-align:center;background:var(--orange);color:#1a100a;letter-spacing:.1em;width:230px;padding:7px;font:900 10px ui-monospace,SFMono-Regular,Menlo,monospace;position:absolute;top:27px;right:-50px;transform:rotate(35deg)}.gc-planned .gc-eyebrow{color:var(--orange)}.gc-planned-heading{justify-content:space-between;align-items:end;gap:24px;padding-right:90px;display:flex}.gc-planned-heading h2{letter-spacing:-.035em;margin:0;font-size:30px}.gc-planned-heading>span{color:var(--orange);text-transform:uppercase;font:700 11px ui-monospace,SFMono-Regular,Menlo,monospace}.gc-planned-intro{color:#c7b7aa;max-width:880px;font-size:14px}.gc-planned-grid{grid-template-columns:repeat(3,1fr);gap:10px;margin-top:24px;display:grid}.gc-planned-item{background:#0c0c0a80;border:1px solid #4b3b30;min-width:0;padding:14px}.gc-planned-item button{color:#8f8379;text-align:left;cursor:not-allowed;background:0 0;border:0;justify-content:space-between;align-items:center;width:100%;padding:0;font-size:13px;font-weight:750;display:flex}.gc-planned-item button span{color:#98735a;text-transform:uppercase;border:1px solid #654a38;padding:3px 6px;font:700 9px ui-monospace,SFMono-Regular,Menlo,monospace}.gc-planned-item p{color:#827870;margin:12px 0 0;font-size:11px;line-height:1.5}.gc-planned-item p strong{color:var(--orange)}.gc-onboarding{max-width:930px}.gc-onboarding h1{font-size:clamp(46px,6vw,70px)}.gc-onboarding-lede{color:var(--muted);font-size:16px}.gc-onboarding-lede code,.gc-repo-heading code{color:var(--cyan)}.gc-gate-card{margin-top:42px}.gc-gate-status{border-bottom:1px solid var(--line);justify-content:space-between;align-items:center;gap:18px;padding:18px 0;display:flex}.gc-gate-status span{color:var(--muted)}.gc-gate-status strong{color:var(--danger)}.gc-gate-status.is-ready strong{color:var(--acid)}.gc-install-panel{padding-top:30px}.gc-install-panel>p:not(.gc-eyebrow){color:#bbb9b0}.gc-install-panel pre{color:#dadad2;white-space:pre-wrap;background:#0b0c0a;border:1px solid #4a4942;max-width:100%;padding:15px;font:11px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace;overflow:auto}.gc-confirm{color:#dfded6;align-items:flex-start;gap:10px;margin:18px 0;font-size:13px;display:flex}.gc-confirm input{accent-color:var(--acid);margin-top:3px}.gc-button{background:var(--acid);cursor:pointer;border:1px solid #59604e;justify-content:center;align-items:center;min-height:42px;padding:0 16px;font-size:12px;font-weight:800;display:inline-flex;color:#12140e!important}.gc-button:hover:not(:disabled){filter:brightness(1.08);transform:translateY(-1px)}.gc-button:disabled{opacity:.45;cursor:not-allowed}.gc-button--quiet{background:#181a16;border-color:#45483f;color:#d5d6ce!important}.gc-button--danger{background:var(--orange);border-color:#b9684b}.gc-setup-actions{flex-wrap:wrap;gap:10px;margin-top:24px;display:flex}.gc-not-supported{border-left:2px solid var(--orange);background:#ff9b550f;padding:13px}.gc-notice{font:700 11px ui-monospace,SFMono-Regular,Menlo,monospace;color:var(--acid)!important}.gc-dashboard{padding-top:44px}.gc-repo-heading{justify-content:space-between;align-items:end;gap:28px;display:flex}.gc-repo-heading h1{letter-spacing:-.05em;margin:0;font-size:clamp(40px,5vw,62px);line-height:1}.gc-repo-heading>div>p:last-child{color:var(--muted);margin-bottom:0}.gc-repo-actions{align-items:center;gap:10px;display:flex}.gc-branch-chip{text-overflow:ellipsis;white-space:nowrap;max-width:280px;color:var(--acid);background:#172013;border:1px solid #405333;padding:11px 13px;font:700 11px ui-monospace,SFMono-Regular,Menlo,monospace;overflow:hidden}.gc-summary-strip{border:1px solid var(--line);grid-template-columns:repeat(4,1fr);margin:38px 0 26px;display:grid}.gc-summary-strip>div{border-right:1px solid var(--line);background:var(--panel);min-width:0;padding:20px}.gc-summary-strip>div:last-child{border:0}.gc-summary-strip span{color:var(--muted);text-transform:uppercase;letter-spacing:.1em;margin-bottom:8px;font:700 10px ui-monospace,SFMono-Regular,Menlo,monospace;display:block}.gc-summary-strip strong{text-overflow:ellipsis;white-space:nowrap;font:750 18px ui-monospace,SFMono-Regular,Menlo,monospace;display:block;overflow:hidden}.gc-delta i,.gc-file-stat i{color:var(--acid);font-style:normal}.gc-delta b,.gc-file-stat b{color:var(--danger);font-weight:inherit;margin-left:8px}.gc-tab-bar{border:1px solid var(--line);background:#11120f;gap:4px;width:max-content;padding:5px;display:flex}.gc-tab-bar button{color:var(--muted);text-transform:capitalize;cursor:pointer;background:0 0;border:0;padding:8px 15px;font-size:12px;font-weight:700}.gc-tab-bar button.is-active{color:var(--acid);background:#30342a}.gc-panel{border:1px solid var(--line);background:var(--panel);margin-top:18px}.gc-panel-heading{border-bottom:1px solid var(--line);justify-content:space-between;align-items:end;gap:20px;padding:25px;display:flex}.gc-panel-heading h2{letter-spacing:-.03em;margin:0;font-size:25px}.gc-panel-heading>span{color:var(--muted);text-transform:uppercase;font:700 10px ui-monospace,SFMono-Regular,Menlo,monospace}.gc-file-list>button,.gc-commit-list>button{text-align:left;cursor:pointer;background:0 0;border:0;border-bottom:1px solid #292c26;grid-template-columns:auto 1fr auto auto;align-items:center;gap:16px;width:100%;padding:17px 22px;display:grid}.gc-file-list>button:last-child,.gc-commit-list>button:last-child{border-bottom:0}.gc-file-list>button:hover,.gc-commit-list>button:hover{background:#22251f}.gc-status-code{width:34px;height:28px;color:var(--orange);background:#2e312a;place-items:center;font:800 11px ui-monospace,SFMono-Regular,Menlo,monospace;display:grid}.gc-status-code--untracked{color:var(--cyan)}.gc-status-code--renamed{color:#cf9cff}.gc-file-main,.gc-commit-main{min-width:0}.gc-file-main strong,.gc-commit-main strong{text-overflow:ellipsis;white-space:nowrap;font-size:13px;display:block;overflow:hidden}.gc-file-main small,.gc-commit-main small{color:var(--muted);margin-top:3px;font-size:11px;display:block}.gc-file-stat{font:700 11px ui-monospace,SFMono-Regular,Menlo,monospace}.gc-file-stat--empty{color:#70726a}.gc-graph-dot{border:2px solid var(--acid);border-radius:50%;width:10px;height:10px;box-shadow:0 0 0 5px #caff4a0f}.gc-commit-list code{color:var(--cyan);font-size:11px}.gc-branch-list>div{border-bottom:1px solid #292c26;grid-template-columns:auto 1fr auto minmax(170px,auto);align-items:center;gap:14px;padding:16px 22px;display:grid}.gc-branch-list>div.is-current{background:#caff4a0d}.gc-branch-mark{color:#666960}.gc-branch-list .is-current .gc-branch-mark{color:var(--acid)}.gc-branch-list code{color:var(--cyan)}.gc-branch-list small{color:var(--muted);text-align:right}.gc-empty-state{text-align:center;color:var(--muted);padding:60px 24px}.gc-empty-state strong{color:var(--ink);font-size:18px}.gc-detail{max-width:1080px;margin:0 auto}.gc-back{cursor:pointer;background:0 0;border:0;margin-bottom:30px;padding:0;font:700 12px ui-monospace,SFMono-Regular,Menlo,monospace;color:var(--acid)!important}.gc-detail-heading{margin-bottom:24px}.gc-detail-heading h2{overflow-wrap:anywhere;letter-spacing:-.04em;max-width:900px;margin:0;font-size:clamp(30px,4vw,50px)}.gc-detail-heading>p:last-child{color:var(--muted);font-size:12px}.gc-detail-heading code{color:var(--cyan)}.gc-diff{color:#d3d6ce;white-space:pre-wrap;tab-size:2;background:#090a08;border:1px solid #3d4137;max-height:65vh;margin:0;padding:24px;font:12px/1.65 ui-monospace,SFMono-Regular,Menlo,monospace;overflow:auto}.gc-about{max-width:1050px}.gc-about h1{max-width:850px}.gc-about-lede{color:#b7b7ae;max-width:760px;font-size:18px;line-height:1.65}.gc-safety-grid{grid-template-columns:repeat(3,1fr);gap:14px;margin:48px 0 28px;display:grid}.gc-safety-grid article{border:1px solid var(--line);background:var(--panel);padding:24px}.gc-safety-grid article>span{color:var(--acid);font:700 11px ui-monospace,SFMono-Regular,Menlo,monospace}.gc-safety-grid h2{font-size:19px}.gc-safety-grid p{color:var(--muted);font-size:13px}.gc-safety-grid code{color:var(--cyan)}.gc-not-found{flex-direction:column;justify-content:center;align-items:flex-start;min-height:500px;display:flex}.gc-not-found .gc-button{margin-top:28px}@media (width<=900px){.gc-header{grid-template-columns:1fr auto;padding:0 18px}.gc-nav{display:none}.gc-hero-grid{grid-template-columns:1fr;gap:36px;padding:30px 0}.gc-picker-card{box-shadow:9px 9px #070807}.gc-planned-grid,.gc-safety-grid,.gc-summary-strip{grid-template-columns:1fr 1fr}.gc-summary-strip>div:nth-child(2){border-right:0}.gc-summary-strip>div:nth-child(-n+2){border-bottom:1px solid var(--line)}.gc-repo-heading{flex-direction:column;align-items:flex-start}}@media (width<=620px){.gc-picker,.gc-dashboard,.gc-onboarding,.gc-about,.gc-not-found{width:min(100% - 28px,1220px);padding-top:40px}.gc-readonly{font-size:0}.gc-readonly i{width:7px;height:7px}.gc-hero-copy h1,.gc-onboarding h1,.gc-about h1,.gc-not-found h1{font-size:43px}.gc-planned{padding:24px 18px}.gc-planned-heading{flex-direction:column;align-items:flex-start;padding-right:0}.gc-planned-heading>span{display:none}.gc-planned-grid,.gc-safety-grid{grid-template-columns:1fr}.gc-repo-actions{flex-direction:column;align-items:stretch;width:100%}.gc-summary-strip{grid-template-columns:1fr}.gc-summary-strip>div{border-right:0;border-bottom:1px solid var(--line)}.gc-file-list>button{grid-template-columns:auto 1fr auto;gap:10px}.gc-file-stat{display:none}.gc-branch-list>div{grid-template-columns:auto 1fr auto}.gc-branch-list small{display:none}}", Ue = "cockpit";
function We(e) {
	if (e.apiVersion !== "1") throw Error(`Git Cockpit requires Canvas host API 1, received ${e.apiVersion}.`);
	return e.registerPage(Ue, ({ container: t, path: n, navigate: r }) => {
		let i = new AbortController(), a = document.createElement("style");
		a.dataset.gitCockpit = "styles", a.textContent = He;
		let o = document.createElement("div");
		o.dataset.gitCockpit = "root", t.append(a, o);
		let s = (0, u.createRoot)(o);
		return s.render(/* @__PURE__ */ (0, k.jsx)(Ve, {
			host: e,
			path: n,
			navigate: r,
			signal: i.signal
...
var Ge = { PAGE_ID: Ue };
//#endregion
export { Ge as __testing, We as activate };
