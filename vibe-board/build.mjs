/* Build the Vibe canvas extension into one self-contained browser ESM file.
   ...
   src/board.css is the board's stylesheet, vendored from the standalone
   vibe-manager SPA. Two transforms adapt it to living inside Canvas:
...
const SOURCE_CSS = join(here, "src", "board.css");
...
const out = readFileSync(join(here, "extension.js"), "utf8");
writeFileSync(
  join(here, "extension.js"),
  `/* Generated by extensions/vibe-board/build.mjs - do not edit. */\n${out}`,
);
console.log(`built extension.js (${(out.length / 1024).toFixed(1)} KB)`);
