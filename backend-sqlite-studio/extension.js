...
var e = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), t = /* @__PURE__ */ e(((e) => {
	...
	e.unstable_IdlePriority = 5, e.unstable_ImmediatePriority = 1, e.unstable_LowPriority = 4, e.unstable_NormalPriority = 3, e.unstable_Profiling = null, e.unstable_UserBlockingPriority = 2, e.unstable_cancelCallback = function(e) {
		...
		return s = a + s, r = {
			...
			expirationTime: s,
			sortIndex: -1
		}, a > o ? (r.sortIndex = a, t(l, r), n(c) === null && r === n(l) && (h ? (v(S), S = -1) : h = !0, T(x, a - o))) : (r.sortIndex = s, t(c, r), m || p || (m = !0, ee || (ee = !0, w()))), r;
	...
	function se(e, r, i, a, o) {
		...
		if (c) return o = o(e), c = a === "" ? "." + T(e, 0) : a, ee(o) ? (i = "", c != null && (i = c.replace(ae, "$&/") + "/"), se(o, r, i, "", function(e) {
			return e;
		})) : o != null && (w(o) && (o = re(o, i + (o.key == null || e && e.key === o.key ? "" : ("" + o.key).replace(ae, "$&/") + "/") + c)), r.push(o)), 1;
		c = 0;
		var l = a === "" ? "." : a + ":";
		if (ee(e)) for (var u = 0; u < e.length; u++) a = e[u], s = l + T(a, u), c += se(a, r, i, s, o);
		else if (u = m(e), typeof u == "function") for (e = u.call(e), u = 0; !(a = e.next()).done;) a = a.value, s = l + T(a, u++), c += se(a, r, i, s, o);
		else if (s === "object") {
	...
	var h = Object.assign, g = Symbol.for("react.element"), _ = Symbol.for("react.transitional.element"), v = Symbol.for("react.portal"), y = Symbol.for("react.fragment"), b = Symbol.for("react.strict_mode"), x = Symbol.for("react.profiler"), ee = Symbol.for("react.consumer"), S = Symbol.for("react.context"), C = Symbol.for("react.forward_ref"), te = Symbol.for("react.suspense"), ne = Symbol.for("react.suspense_list"), re = Symbol.for("react.memo"), w = Symbol.for("react.lazy"), ie = Symbol.for("react.activity"), ae = Symbol.for("react.memo_cache_sentinel"), T = Symbol.iterator;
	function oe(e) {
		...
		switch (t.lanes = 0, t.tag) {
			...
			case 3:
				a: {
					...
					if (r = o.cache, na(t, M, r), r !== a.cache && aa(t, [M], n, !0), to(), r = o.element, a.isDehydrated) {
						if (a = {
							element: r,
							isDehydrated: !1,
							cache: o.cache
...
function d(e) {
	return `backend-sqlite-studio:audit:${e.id ?? "unknown"}`;
}
function f(e) {
	try {
		let t = JSON.parse(localStorage.getItem(d(e)) ?? "[]");
		return Array.isArray(t) ? t.slice(0, 30) : [];
	} catch {
		return [];
	}
}
function p(e, t) {
	...
	return localStorage.setItem(d(e), JSON.stringify(n)), n;
}
//#endregion
//#region src/backend-service.ts
var m = ".openhands/apps/backend-sqlite-studio", h = "studio.sqlite3", g = String.raw`from __future__ import annotations
...
var y = _(g), b = _(JSON.stringify({ action: "initialize" })), x = String.raw`python_ready=0
python_version=''
python_sqlite=''
sqlite_cli=''
...
printf 'BACKEND_SQLITE_STUDIO_PROBE\t%s\n' "$probe64"`, ee = String.raw`app_dir='.openhands/apps/backend-sqlite-studio'
mkdir -p "$app_dir"
wrapper64='${y}'
...
printf '%s' '${"1"}' > "$app_dir/.runtime-version"
python3 "$app_dir/wrapper.py" '${b}'`, S = String.raw`python3 -c 'import pathlib, shutil; base=pathlib.Path.cwd().resolve(); target=(base/".openhands/apps/backend-sqlite-studio").resolve(); expected=(base/".openhands/apps/backend-sqlite-studio").resolve(); target == expected or (_ for _ in ()).throw(RuntimeError("unsafe reset target")); shutil.rmtree(target) if target.exists() else None; print("BACKEND_SQLITE_STUDIO_RESET")'`, C = "Work with the Backend SQLite Studio database on this Agent Server. The exact database path is";
function te(e) {
	if (e?.aborted) throw new DOMException("The Agent Server request was cancelled.", "AbortError");
}
function ne(e) {
	return typeof e != "string" || e.length < 2 || e.length > 4096 || !e.startsWith("/") || /[\0-\x1f\x7f]/.test(e) || e.includes("//") ? !1 : e.split("/").filter(Boolean).every((e) => e !== "." && e !== "..");
}
function re(e) {
	if (!ne(e)) throw Error("The Agent Server returned an unsafe home path.");
	return `${e.replace(/\/+$/, "")}/${m}`;
}
async function w(e, t) {
	te(t);
	let n = await e.agentServer.request({ path: "/api/file/home" });
	te(t);
	let r;
	try {
		r = typeof n == "string" ? JSON.parse(n) : n;
	} catch {
		throw Error("The Agent Server home response was not valid JSON.");
	}
	if (!ne(r?.home)) throw Error("The Agent Server did not report a safe absolute home directory.");
	return r.home;
}
async function ie(e, t, n, r, i) {
	if (!ne(t)) throw Error("The Agent Server home path failed validation.");
	te(i);
	...
	te(i);
	let o = typeof a?.stdout == "string" ? a.stdout : "", s = typeof a?.stderr == "string" ? a.stderr.trim() : "", c = typeof a?.exit_code == "number" ? a.exit_code : -1;
	if (c !== 0) throw Error(s || `The controlled backend command failed with exit code ${c}.`);
	return o;
}
function ae(e) {
	let t = e.match(/^BACKEND_SQLITE_STUDIO\t([A-Za-z0-9+/=]+)$/m);
	if (!t) throw Error("The SQLite wrapper returned an unexpected response.");
	let n;
	try {
		n = JSON.parse(v(t[1]));
	} catch {
		throw Error("The SQLite wrapper response could not be decoded.");
	}
	if (!n.ok) throw Error(n.error || "The SQLite wrapper failed.");
	return n.data;
}
function T(e) {
	let t = _(JSON.stringify(e));
	if (!/^[A-Za-z0-9+/=]+$/.test(t)) throw Error("The structured SQLite payload could not be safely encoded.");
	return `python3 '${m}/wrapper.py' '${t}'`;
}
async function oe(e, t, n) {
	let r = (await ie(e, t, x, 15, n)).match(/^BACKEND_SQLITE_STUDIO_PROBE\t([A-Za-z0-9+/=]+)$/m);
	if (!r) throw Error("The prerequisite probe returned an unexpected response.");
	try {
		let e = JSON.parse(v(r[1]));
		return {
			pythonReady: e.pythonReady === 1,
			pythonVersion: typeof e.pythonVersion == "string" && e.pythonVersion ? e.pythonVersion : null,
			sqliteModuleVersion: typeof e.sqliteModuleVersion == "string" && e.sqliteModuleVersion ? e.sqliteModuleVersion : null,
			sqliteCliVersion: typeof e.sqliteCliVersion == "string" && e.sqliteCliVersion ? e.sqliteCliVersion : null,
			initialized: e.initialized === 1,
			wrapperPresent: e.wrapperPresent === 1,
			databasePresent: e.databasePresent === 1,
			runtimeVersion: typeof e.runtimeVersion == "string" && e.runtimeVersion ? e.runtimeVersion : null
		};
	} catch {
		throw Error("The prerequisite probe returned malformed data.");
	}
}
async function se(e, t, n) {
	return ae(await ie(e, t, ee, 30, n));
}
async function ce(e, t, n) {
	return ae(await ie(e, t, T({ action: "status" }), 20, n));
}
function le(e) {
	let t = [], n = "", r = "", i = !1, a = !1;
	...
	if (r || a) throw Error("The SQL editor contains an unterminated quote or comment.");
	return n.trim() && t.push(n.trim()), t;
}
async function E(e, t, n, r) {
	let i = le(n);
	if (!i.length) throw Error("Enter a SQL statement before running the query.");
	return ae(await ie(e, t, T({
		action: "query",
		sql: n,
		statements: i
	}), 60, r));
...
async function D(e, t, n) {
	let r = ae(await ie(e, t, T({ action: "export" }), 30, n));
	return {
		bytes: Uint8Array.from(atob(r.databaseBase64), (e) => e.charCodeAt(0)),
		sizeBytes: r.sizeBytes
	};
}
async function ue(e, t, n, r) {
	if (n.byteLength > 16777216) throw Error("Import is limited to 16 MiB.");
	let i = "";
	for (let e of n) i += String.fromCharCode(e);
	return ae(await ie(e, t, T({
		action: "import",
		databaseBase64: btoa(i)
	}), 60, r));
}
async function de(e, t, n) {
	if (!(await ie(e, t, S, 20, n)).includes("BACKEND_SQLITE_STUDIO_RESET")) throw Error("The reset command returned an unexpected response.");
}
function fe(e) {
...
function pe(e) {
	return `Set up the prerequisite for the Backend SQLite Studio App on this Agent Server. The App's proposed data directory is ${re(e)}, but do not create, replace, reset, or delete that directory yourself. First inspect whether python3 can import the standard sqlite3 module and report both Python and SQLite versions. If that runtime is missing, identify the operating system and package manager, explain the exact install command and files it may change, and obtain my approval before installing Python 3 with SQLite support. Do not install anything if the runtime is already usable. Verify with "python3 -c 'import sqlite3; print(sqlite3.sqlite_version)'", then tell me to use Recheck and the deliberate Install or Repair action in the App.`;
}
//#endregion
...
var O = /* @__PURE__ */ e(((e) => {
	...
	t.exports = O();
})))(), me = "/extensions/backend-sqlite-studio/studio", he = "/extensions/backend-sqlite-studio/handoff", ge = "SELECT id, title, body, created_at\nFROM notes\nORDER BY id DESC\nLIMIT 100;";
function _e(e, t) {
	let n = t.replace(/^\/+|\/+$/g, "").split("/").filter(Boolean)[0] ?? "";
	return e === "studio" && (n === "" || n === "studio") ? "studio" : e === "handoff" && (n === "" || n === "handoff") ? "handoff" : "not-found";
}
function ve(e) {
...
function be(e) {
	return e === null ? "NULL" : typeof e == "object" ? `[BLOB · ${e.blobBytes} bytes]` : String(e);
}
function xe({ view: e, navigate: t, ready: n }) {
	return /* @__PURE__ */ (0, k.jsxs)("header", {
		...
		children: [
			/* @__PURE__ */ (0, k.jsxs)("button", {
				...
				}), /* @__PURE__ */ (0, k.jsxs)("span", { children: [/* @__PURE__ */ (0, k.jsx)("small", { children: "AGENT SERVER DATA" }), /* @__PURE__ */ (0, k.jsx)("strong", { children: "SQLite Studio" })] })]
			}),
			/* @__PURE__ */ (0, k.jsxs)("nav", {
				"aria-label": "Backend SQLite Studio pages",
				children: [/* @__PURE__ */ (0, k.jsx)("button", {
					type: "button",
					"aria-current": e === "studio" ? "page" : void 0,
					onClick: () => t(me),
					children: "Studio"
				}), /* @__PURE__ */ (0, k.jsx)("button", {
					type: "button",
					"aria-current": e === "handoff" ? "page" : void 0,
					onClick: () => t(he),
					children: "Agent handoff"
				})]
			}),
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: `bss-status ${n ? "is-ready" : ""}`,
				children: [/* @__PURE__ */ (0, k.jsx)("span", {}), n ? "Backend ready" : "Setup needed"]
			})
		]
	});
}
function Se({ home: e, probe: t, busy: n, onInstall: r, onRecheck: i, onRepair: a, onCopy: o, copyNotice: s, audit: c }) {
	let [u, d] = (0, l.useState)(!1), f = re(e), p = t.wrapperPresent || t.databasePresent || t.runtimeVersion !== null;
	return /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "bss-setup",
		children: [
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "bss-setup-hero",
				children: [
					/* @__PURE__ */ (0, k.jsx)("span", {
						className: "bss-eyebrow",
						children: "FIRST-RUN CHECK"
					}),
					/* @__PURE__ */ (0, k.jsx)("h1", { children: "Give this database a durable home." }),
					/* @__PURE__ */ (0, k.jsx)("p", { children: "The probe only inspected installed commands and file presence. It did not create directories, download software, or open a database." })
				]
			}),
			/* @__PURE__ */ (0, k.jsxs)("section", {
				className: "bss-setup-grid",
				children: [
					/* @__PURE__ */ (0, k.jsxs)("article", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							className: t.pythonReady ? "ok" : "warn",
							children: t.pythonReady ? "READY" : "MISSING"
						}),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: "Python SQLite runtime" }),
						/* @__PURE__ */ (0, k.jsx)("p", { children: t.pythonVersion ?? "python3 was not detected" }),
						/* @__PURE__ */ (0, k.jsx)("code", { children: t.sqliteModuleVersion ? `SQLite ${t.sqliteModuleVersion}` : "sqlite3 module unavailable" })
					] }),
					/* @__PURE__ */ (0, k.jsxs)("article", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							className: t.sqliteCliVersion ? "ok" : "neutral",
							children: t.sqliteCliVersion ? "DETECTED" : "OPTIONAL"
						}),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: "sqlite3 CLI" }),
						/* @__PURE__ */ (0, k.jsx)("p", { children: t.sqliteCliVersion ?? "Not installed; the App does not require it." }),
						/* @__PURE__ */ (0, k.jsx)("small", { children: "The App uses Python’s standard sqlite3 module so no database engine is downloaded." })
					] }),
					/* @__PURE__ */ (0, k.jsxs)("article", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							className: p ? "warn" : "neutral",
							children: p ? "PARTIAL" : "PROPOSED"
						}),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: "App-owned directory" }),
						/* @__PURE__ */ (0, k.jsx)("code", { children: f }),
						/* @__PURE__ */ (0, k.jsxs)("p", { children: [
							"Install writes ",
							/* @__PURE__ */ (0, k.jsx)("strong", { children: "wrapper.py" }),
							", a runtime marker, and ",
							/* @__PURE__ */ (0, k.jsx)("strong", { children: h }),
							" only here."
						] })
					] })
				]
			}),
			t.pythonReady ? null : /* @__PURE__ */ (0, k.jsxs)("section", {
				className: "bss-callout is-warning",
				role: "alert",
				children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "A compatible runtime is required." }), /* @__PURE__ */ (0, k.jsx)("p", { children: "Copy the setup prompt and ask an OpenHands agent to inspect the operating system, explain any package changes, obtain approval, and install Python 3 with SQLite support. Then recheck." })]
			}),
			/* @__PURE__ */ (0, k.jsxs)("section", {
				className: "bss-consent",
				children: [
					/* @__PURE__ */ (0, k.jsxs)("div", { children: [
						/* @__PURE__ */ (0, k.jsx)("span", {
							className: "bss-eyebrow",
							children: "DELIBERATE SETUP"
						}),
						/* @__PURE__ */ (0, k.jsx)("h2", { children: p ? "Repair the App-owned runtime" : "Initialize Backend SQLite Studio" }),
						/* @__PURE__ */ (0, k.jsx)("p", { children: "This creates or replaces the App’s wrapper and migration marker, creates the database if absent, and applies versioned migrations. Repair preserves existing database rows." })
					] }),
					/* @__PURE__ */ (0, k.jsxs)("label", { children: [/* @__PURE__ */ (0, k.jsx)("input", {
						type: "checkbox",
						checked: u,
						onChange: (e) => d(e.target.checked)
					}), " I understand the files and target shown above."] }),
					/* @__PURE__ */ (0, k.jsxs)("div", {
						className: "bss-actions",
						children: [
							/* @__PURE__ */ (0, k.jsx)("button", {
								type: "button",
								className: "bss-button primary",
								disabled: !u || !t.pythonReady || !!n,
								onClick: p ? a : r,
								children: n || (p ? "Repair" : "Install")
							}),
							/* @__PURE__ */ (0, k.jsx)("button", {
								type: "button",
			...
			/* @__PURE__ */ (0, k.jsx)(Te, { entries: c })
		]
	});
}
function Ce({ snapshot: e, restore: t }) {
	return /* @__PURE__ */ (0, k.jsxs)("aside", {
		className: "bss-sidebar",
		children: [
			/* @__PURE__ */ (0, k.jsxs)("section", { children: [/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "bss-side-title",
				children: [/* @__PURE__ */ (0, k.jsx)("h2", { children: "Schema" }), /* @__PURE__ */ (0, k.jsx)("span", { children: e.tables.length })]
			}), e.tables.map((e) => /* @__PURE__ */ (0, k.jsxs)("details", {
				open: !0,
				children: [/* @__PURE__ */ (0, k.jsxs)("summary", { children: [
					"▦ ",
					e.name,
					/* @__PURE__ */ (0, k.jsx)("span", { children: e.columns.length })
				] }), /* @__PURE__ */ (0, k.jsx)("ul", { children: e.columns.map((e) => /* @__PURE__ */ (0, k.jsxs)("li", { children: [/* @__PURE__ */ (0, k.jsx)("code", { children: e.name }), /* @__PURE__ */ (0, k.jsxs)("small", { children: [
					e.type,
					e.primaryKey ? " · PK" : "",
					e.nullable ? "" : " · required"
				] })] }, e.name)) })]
			...
			/* @__PURE__ */ (0, k.jsxs)("section", { children: [/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "bss-side-title",
				children: [/* @__PURE__ */ (0, k.jsx)("h2", { children: "Migrations" }), /* @__PURE__ */ (0, k.jsx)("span", { children: e.migrations.length })]
			}), e.migrations.map((e) => /* @__PURE__ */ (0, k.jsxs)("div", {
				className: "bss-migration",
				children: [/* @__PURE__ */ (0, k.jsx)("i", {}), /* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: e.id }), /* @__PURE__ */ (0, k.jsx)("small", { children: new Date(e.appliedAt).toLocaleString() })] })]
			}, e.id))] }),
			/* @__PURE__ */ (0, k.jsxs)("section", {
				className: "bss-history",
				children: [/* @__PURE__ */ (0, k.jsxs)("div", {
					className: "bss-side-title",
					children: [/* @__PURE__ */ (0, k.jsx)("h2", { children: "Query history" }), /* @__PURE__ */ (0, k.jsx)("span", { children: e.history.length })]
				}), e.history.map((e) => /* @__PURE__ */ (0, k.jsxs)("button", {
					type: "button",
					onClick: () => t(e.sql),
					title: e.message || e.sql,
					children: [/* @__PURE__ */ (0, k.jsx)("i", { className: e.status }), /* @__PURE__ */ (0, k.jsxs)("span", { children: [/* @__PURE__ */ (0, k.jsx)("code", { children: e.sql.replace(/\s+/g, " ") }), /* @__PURE__ */ (0, k.jsxs)("small", { children: [
						e.status,
						" · ",
...
function we({ response: e }) {
	return e ? e.results.length ? /* @__PURE__ */ (0, k.jsx)("div", { children: e.results.map((t, n) => /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "bss-result",
		children: [
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "bss-result-meta",
				children: [/* @__PURE__ */ (0, k.jsxs)("span", { children: ["Result ", n + 1] }), /* @__PURE__ */ (0, k.jsxs)("span", { children: [
					t.values.length,
					t.truncated ? "+" : "",
					" rows · ",
			...
			/* @__PURE__ */ (0, k.jsx)("div", {
				className: "bss-table-wrap",
				children: /* @__PURE__ */ (0, k.jsxs)("table", { children: [/* @__PURE__ */ (0, k.jsx)("thead", { children: /* @__PURE__ */ (0, k.jsx)("tr", { children: t.columns.map((e) => /* @__PURE__ */ (0, k.jsx)("th", { children: e }, e)) }) }), /* @__PURE__ */ (0, k.jsx)("tbody", { children: t.values.map((e, t) => /* @__PURE__ */ (0, k.jsx)("tr", { children: e.map((e, t) => /* @__PURE__ */ (0, k.jsx)("td", {
					className: e === null ? "is-null" : void 0,
					children: be(e)
				}, t)) }, t)) })] })
			}),
			t.truncated ? /* @__PURE__ */ (0, k.jsx)("p", {
				className: "bss-limit",
				children: "Showing the first 250 rows."
			}) : null
		]
	}, n)) }) : /* @__PURE__ */ (0, k.jsxs)("div", {
		className: "bss-summary",
		children: [
			"Completed in ",
			e.durationMs.toFixed(1),
			" ms · ",
		...
		className: "bss-empty",
		children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Results land here" }), /* @__PURE__ */ (0, k.jsx)("p", { children: "Run the starter query or create your own tables inside this database." })]
	});
}
function Te({ entries: e }) {
	return /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "bss-audit",
		children: [/* @__PURE__ */ (0, k.jsxs)("div", {
			className: "bss-panel-heading",
			children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", {
				className: "bss-eyebrow",
				children: "THIS BROWSER · THIS BACKEND"
			}), /* @__PURE__ */ (0, k.jsx)("h2", { children: "Action record" })] }), /* @__PURE__ */ (0, k.jsx)("span", { children: e.length })]
		}), e.length ? /* @__PURE__ */ (0, k.jsx)("ol", { children: e.map((e) => /* @__PURE__ */ (0, k.jsxs)("li", { children: [
			/* @__PURE__ */ (0, k.jsx)("i", { className: e.status }),
			/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: e.action }), /* @__PURE__ */ (0, k.jsx)("p", { children: e.detail })] }),
			/* @__PURE__ */ (0, k.jsx)("time", { children: new Date(e.at).toLocaleString() })
		] }, e.id)) }) : /* @__PURE__ */ (0, k.jsx)("p", {
			className: "bss-empty-line",
			children: "No mutating setup or data actions recorded in this browser yet."
		})]
	});
}
function Ee({ host: e, home: t, snapshot: n, setSnapshot: r, audit: i, setAudit: a, signal: o, recheck: s, onRepair: c }) {
	let [u, d] = (0, l.useState)(ge), [f, m] = (0, l.useState)(null), [g, _] = (0, l.useState)(""), [v, y] = (0, l.useState)(""), [b, x] = (0, l.useState)(""), [ee, S] = (0, l.useState)(!1), [C, te] = (0, l.useState)(null), [ne, w] = (0, l.useState)(!1), [ie, ae] = (0, l.useState)(!1), T = (0, l.useRef)(/* @__PURE__ */ new Set()), oe = re(t), se = `${oe}/${h}`;
	(0, l.useEffect)(() => () => {
		for (let e of T.current) URL.revokeObjectURL(e);
		T.current.clear();
	}, []);
	let ce = (0, l.useCallback)((t, n, r = "success") => a(p(e.backend, {
		action: t,
		detail: n,
		status: r
	})), [e.backend, a]), le = (0, l.useCallback)(async (e, t) => {
		_(e), x(""), y("");
		try {
			await t();
		} catch (t) {
	...
	}, [ce]), fe = () => void le("Run SQL", async () => {
		let n = await E(e, t, u, o);
		r(n), m(n), y("SQL completed on the Agent Server database."), ce("Run SQL", `${n.rowsAffected} rows changed · ${u.replace(/\s+/g, " ").slice(0, 180)}`);
	});
	return /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "bss-workspace",
		children: [/* @__PURE__ */ (0, k.jsx)(Ce, {
			...
			children: [
				/* @__PURE__ */ (0, k.jsxs)("section", {
					className: "bss-boundary",
					children: [
						/* @__PURE__ */ (0, k.jsx)("span", { children: "◎" }),
						/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Agent Server persistence boundary" }), /* @__PURE__ */ (0, k.jsx)("p", { children: "Shared database data lives on the backend. Editor text and this action record stay in this browser." })] }),
						/* @__PURE__ */ (0, k.jsx)("code", {
							title: se,
							children: se
						})
					]
				...
				/* @__PURE__ */ (0, k.jsxs)("section", {
					className: "bss-status-grid",
					children: [
						/* @__PURE__ */ (0, k.jsxs)("article", { children: [
							/* @__PURE__ */ (0, k.jsx)("small", { children: "DATABASE" }),
							/* @__PURE__ */ (0, k.jsxs)("strong", { children: [(n.sizeBytes / 1024).toFixed(1), " KiB"] }),
							/* @__PURE__ */ (0, k.jsxs)("span", { children: [n.journalMode, " journal"] })
						] }),
						/* @__PURE__ */ (0, k.jsxs)("article", { children: [
							/* @__PURE__ */ (0, k.jsx)("small", { children: "TABLES" }),
							/* @__PURE__ */ (0, k.jsx)("strong", { children: n.tables.length }),
							/* @__PURE__ */ (0, k.jsx)("span", { children: "user-visible" })
						] }),
						/* @__PURE__ */ (0, k.jsxs)("article", { children: [
							/* @__PURE__ */ (0, k.jsx)("small", { children: "MIGRATIONS" }),
							/* @__PURE__ */ (0, k.jsx)("strong", { children: n.migrations.length }),
							/* @__PURE__ */ (0, k.jsx)("span", { children: "applied" })
						] })
					]
				}),
				/* @__PURE__ */ (0, k.jsxs)("section", {
					...
					children: [
						/* @__PURE__ */ (0, k.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", {
								className: "bss-eyebrow",
								children: "STRUCTURED WRAPPER"
							}), /* @__PURE__ */ (0, k.jsx)("h1", { children: "SQL editor" })] }), /* @__PURE__ */ (0, k.jsxs)("div", {
								className: "bss-actions",
								children: [
									/* @__PURE__ */ (0, k.jsx)("button", {
										className: "bss-button",
									...
									/* @__PURE__ */ (0, k.jsx)("button", {
										...
										type: "button",
										disabled: !!g,
										onClick: () => void le("Export database", async () => {
											let n = await D(e, t, o), r = URL.createObjectURL(new Blob([n.bytes], { type: "application/vnd.sqlite3" }));
											T.current.add(r);
											let i = document.createElement("a");
											i.href = r, i.download = `backend-sqlite-studio-${Date.now()}.sqlite3`, i.click(), window.setTimeout(() => {
												URL.revokeObjectURL(r), T.current.delete(r);
											}, 0), y(`Exported ${n.sizeBytes.toLocaleString()} bytes.`);
										}),
										children: "Export"
									}),
									/* @__PURE__ */ (0, k.jsxs)("label", {
										className: "bss-button file",
										children: ["Import", /* @__PURE__ */ (0, k.jsx)("input", {
											type: "file",
											accept: ".sqlite,.sqlite3,.db,application/vnd.sqlite3",
											onChange: (e) => {
												te(e.target.files?.[0] ?? null), w(!!e.target.files?.[0]);
											}
										})]
									}),
									/* @__PURE__ */ (0, k.jsx)("button", {
										className: "bss-button danger",
										type: "button",
										onClick: () => S(!0),
										children: "Reset data"
									})
						...
						/* @__PURE__ */ (0, k.jsx)("textarea", {
							...
							onKeyDown: (e) => {
								(e.metaKey || e.ctrlKey) && e.key === "Enter" && (e.preventDefault(), fe());
							},
							spellCheck: !1
						}),
						/* @__PURE__ */ (0, k.jsxs)("div", {
							className: "bss-editor-footer",
							children: [/* @__PURE__ */ (0, k.jsx)("span", { children: "SQL becomes base64 JSON—not shell source. External-file and transaction-control statements are blocked." }), /* @__PURE__ */ (0, k.jsx)("button", {
								className: "bss-button primary",
								type: "button",
								disabled: !!g,
								onClick: fe,
								children: g === "Run SQL" ? "Running…" : "Run SQL"
							})]
						})
					]
				}),
				ne && C ? /* @__PURE__ */ (0, k.jsxs)("section", {
					className: "bss-confirm",
					role: "alertdialog",
					"aria-labelledby": "import-title",
					children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsxs)("strong", {
						id: "import-title",
						children: [
							"Replace the database with ",
							C.name,
							"?"
						]
					}), /* @__PURE__ */ (0, k.jsxs)("p", { children: [
						"This overwrites ",
						/* @__PURE__ */ (0, k.jsx)("code", { children: se }),
						" after a SQLite integrity check. The existing file cannot be restored by this App."
					] })] }), /* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("button", {
						className: "bss-button",
						type: "button",
						onClick: () => {
							w(!1), te(null);
						...
						className: "bss-button danger-solid",
						type: "button",
						onClick: () => void le("Import database", async () => {
							if (!C) throw Error("Choose a SQLite file first.");
							if (C.size > 16777216) throw Error("Import is limited to 16 MiB.");
							let n = new Uint8Array(await C.arrayBuffer());
							r(await ue(e, t, n, o)), m(null), w(!1), te(null), y("Imported database, verified integrity, and applied migrations."), ce("Import database", `Replaced ${se} with ${n.byteLength.toLocaleString()} validated bytes.`);
						}),
						children: "Import and replace"
					})] })]
				}) : null,
				ie ? /* @__PURE__ */ (0, k.jsxs)("section", {
					className: "bss-confirm is-repair",
					role: "alertdialog",
					"aria-labelledby": "repair-title",
					children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("strong", {
						id: "repair-title",
						children: "Repair the structured wrapper?"
					}), /* @__PURE__ */ (0, k.jsxs)("p", { children: [
						"This replaces ",
						/* @__PURE__ */ (0, k.jsxs)("code", { children: [oe, "/wrapper.py"] }),
						" and the runtime marker, then reapplies pending migrations. Existing database rows are preserved."
					] })] }), /* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("button", {
						className: "bss-button",
						type: "button",
						onClick: () => ae(!1),
						...
						children: "Repair wrapper"
					})] })]
				}) : null,
				ee ? /* @__PURE__ */ (0, k.jsxs)("section", {
					className: "bss-confirm",
					role: "alertdialog",
					"aria-labelledby": "reset-title",
					children: [/* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("strong", {
						id: "reset-title",
						children: "Delete all Backend SQLite Studio data?"
					}), /* @__PURE__ */ (0, k.jsxs)("p", { children: [
						"The exact deletion target is ",
						/* @__PURE__ */ (0, k.jsx)("code", { children: oe }),
						". This removes the database, history, migrations, and wrapper; it cannot be undone."
					] })] }), /* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("button", {
						className: "bss-button",
						type: "button",
						onClick: () => S(!1),
						...
						className: "bss-button danger-solid",
						type: "button",
						onClick: () => void le("Reset data", async () => {
							await de(e, t, o), ce("Reset data", `Deleted only ${oe}.`), S(!1), m(null), s();
						}),
						children: "Delete exact directory"
					})] })]
				}) : null,
				b ? /* @__PURE__ */ (0, k.jsxs)("div", {
					className: "bss-message error",
					role: "alert",
					children: [/* @__PURE__ */ (0, k.jsx)("strong", { children: "Action failed" }), /* @__PURE__ */ (0, k.jsx)("span", { children: b })]
				}) : null,
				v ? /* @__PURE__ */ (0, k.jsxs)("div", {
					className: "bss-message success",
					role: "status",
					children: ["✓ ", v]
				}) : null,
				/* @__PURE__ */ (0, k.jsxs)("section", {
					...
					children: [/* @__PURE__ */ (0, k.jsx)("div", {
						...
						children: /* @__PURE__ */ (0, k.jsxs)("div", { children: [/* @__PURE__ */ (0, k.jsx)("span", {
							className: "bss-eyebrow",
							children: "OUTPUT"
						}), /* @__PURE__ */ (0, k.jsx)("h2", { children: "Results" })] })
					}), /* @__PURE__ */ (0, k.jsx)(we, { response: f })]
				}),
				/* @__PURE__ */ (0, k.jsx)(Te, { entries: i })
			]
		})]
	});
}
function De({ home: e, copy: t }) {
	let n = fe(e), r = `${re(e)}/${h}`;
	return /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "bss-handoff",
		children: [
			/* @__PURE__ */ (0, k.jsx)("span", {
				className: "bss-eyebrow",
				children: "ONE DATABASE · MANY CLIENTS"
			}),
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "Hand durable context to an agent." }),
			/* @__PURE__ */ (0, k.jsx)("p", { children: "The database is on the selected Agent Server, so another Canvas client or an OpenHands agent can work with the same rows. Browser editor text and the browser action record are separate and are not shared." }),
			/* @__PURE__ */ (0, k.jsxs)("section", { children: [
				/* @__PURE__ */ (0, k.jsx)("small", { children: "EXACT DATABASE PATH" }),
				/* @__PURE__ */ (0, k.jsx)("code", { children: r }),
				/* @__PURE__ */ (0, k.jsx)("h2", { children: "Safe agent instructions" }),
				/* @__PURE__ */ (0, k.jsx)("pre", { children: n }),
				/* @__PURE__ */ (0, k.jsx)("button", {
					className: "bss-button primary",
					type: "button",
					onClick: t,
					children: "Copy agent handoff"
				})
			] })
		]
	});
}
function Oe({ host: e, pageId: t, path: n, navigate: r, signal: i }) {
	let a = _e(t, n), [o, s] = (0, l.useState)(""), [c, u] = (0, l.useState)(null), [d, m] = (0, l.useState)(null), [h, g] = (0, l.useState)(""), [_, v] = (0, l.useState)(""), [y, b] = (0, l.useState)(""), [x, ee] = (0, l.useState)(0), [S, C] = (0, l.useState)(() => f(e.backend)), te = !!(c?.initialized && d);
	(0, l.useEffect)(() => {
		g(""), u(null), m(null), w(e, i).then(async (t) => {
			s(t);
			let n = await oe(e, t, i);
			u(n), n.initialized && m(await ce(e, t, i));
		}).catch((e) => {
			ye(e) || g(ve(e));
		});
	}, [
	...
	let ne = (0, l.useCallback)(() => ee((e) => e + 1), []), ie = (0, l.useCallback)(async () => {
		o && (await navigator.clipboard.writeText(fe(o)), b("Prompt copied."));
	}, [o]), ae = (0, l.useCallback)(async () => {
		o && (await navigator.clipboard.writeText(pe(o)), b("Setup prompt copied."));
	}, [o]), T = (0, l.useCallback)(async (t) => {
		v(`${t}ing…`), g("");
		try {
			let n = await se(e, o, i);
			m(n), u(await oe(e, o, i)), C(p(e.backend, {
				action: t,
				detail: `${t} completed in ${re(o)}.`,
				status: "success"
			}));
		} catch (n) {
			if (!ye(n)) {
		...
		e,
		i
	]), le = (0, l.useMemo)(() => a === "not-found" ? /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "bss-state",
		children: [
			/* @__PURE__ */ (0, k.jsx)("span", {
				className: "bss-eyebrow",
			...
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "This view is not part of Backend SQLite Studio." }),
			/* @__PURE__ */ (0, k.jsx)("button", {
				className: "bss-button primary",
				onClick: () => r(me),
	...
	}) : h && !c ? /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "bss-state error-state",
		role: "alert",
		children: [
			/* @__PURE__ */ (0, k.jsx)("span", { children: "!" }),
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "Backend discovery failed." }),
			/* @__PURE__ */ (0, k.jsx)("p", { children: h }),
			/* @__PURE__ */ (0, k.jsx)("button", {
				className: "bss-button primary",
				onClick: ne,
				children: "Try again"
	...
	}) : !o || !c ? /* @__PURE__ */ (0, k.jsxs)("main", {
		className: "bss-state",
		children: [
			/* @__PURE__ */ (0, k.jsxs)("div", {
				className: "bss-loader",
				children: [
					/* @__PURE__ */ (0, k.jsx)("i", {}),
					/* @__PURE__ */ (0, k.jsx)("i", {}),
					/* @__PURE__ */ (0, k.jsx)("i", {})
			...
			/* @__PURE__ */ (0, k.jsx)("span", {
				className: "bss-eyebrow",
				children: "NON-MUTATING PROBE"
			}),
			/* @__PURE__ */ (0, k.jsx)("h1", { children: "Inspecting the Agent Server…" })
		]
	}) : a === "handoff" ? /* @__PURE__ */ (0, k.jsx)(De, {
		home: o,
		copy: ie
	}) : !c.initialized || !d ? /* @__PURE__ */ (0, k.jsxs)(k.Fragment, { children: [/* @__PURE__ */ (0, k.jsx)(Se, {
		home: o,
		probe: c,
		busy: _,
		onInstall: () => void T("Install"),
		onRepair: () => void T("Repair"),
		onRecheck: ne,
		onCopy: () => void ae(),
		copyNotice: y,
		audit: S
	}), h ? /* @__PURE__ */ (0, k.jsx)("div", {
		className: "bss-message error setup-error",
		role: "alert",
		children: h
	}) : null] }) : /* @__PURE__ */ (0, k.jsx)(Ee, {
		host: e,
		home: o,
		snapshot: d,
		setSnapshot: m,
	...
	return /* @__PURE__ */ (0, k.jsxs)("section", {
		className: "backend-sqlite-studio",
		"aria-label": "Backend SQLite Studio App",
		children: [
			/* @__PURE__ */ (0, k.jsx)(xe, {
				view: a,
				navigate: r,
			...
			/* @__PURE__ */ (0, k.jsxs)("footer", { children: [/* @__PURE__ */ (0, k.jsxs)("span", { children: [
				...
				" / ",
				e.backend.id ?? "unavailable"
			] }), /* @__PURE__ */ (0, k.jsx)("span", { children: "Browser preferences remain client-local" })] })
		]
	});
}
//#endregion
//#region src/styles.css?inline
var ke = ".backend-sqlite-studio{--bss-ink:#17211e;--bss-muted:#66726c;--bss-line:#d8ddd7;--bss-paper:#fffdf8;--bss-bg:#f1f3ed;--bss-green:#146b4b;--bss-green-dark:#0b4934;--bss-green-soft:#e4f2ea;--bss-coral:#c8523f;min-height:100%;color:var(--bss-ink);background:radial-gradient(circle at 88% -8%, #1b815921, transparent 31rem), var(--bss-bg);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;font-size:14px;line-height:1.45}.backend-sqlite-studio *,.backend-sqlite-studio :before,.backend-sqlite-studio :after{box-sizing:border-box}.backend-sqlite-studio button,.backend-sqlite-studio textarea,.backend-sqlite-studio input{font:inherit}.backend-sqlite-studio code,.backend-sqlite-studio pre,.backend-sqlite-studio textarea{font-family:SFMono-Regular,Consolas,Liberation Mono,monospace}.bss-header{border-bottom:1px solid var(--bss-line);z-index:10;-webkit-backdrop-filter:blur(16px);backdrop-filter:blur(16px);background:#fffdf8eb;grid-template-columns:minmax(240px,1fr) auto minmax(240px,1fr);align-items:center;gap:24px;min-height:72px;padding:12px 28px;display:grid;position:sticky;top:0}.bss-brand{width:fit-content;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;align-items:center;gap:12px;padding:0;display:flex}.bss-brand>span:last-child{flex-direction:column;display:flex}.bss-brand small{color:var(--bss-green);letter-spacing:.17em;font-size:8px;font-weight:850}.bss-brand strong{font-family:Georgia,serif;font-size:20px;line-height:1.15}.bss-mark{place-items:center;width:36px;height:36px;display:grid;position:relative}.bss-mark i{border:2px solid var(--bss-green);background:var(--bss-paper);border-radius:50%;width:30px;height:12px;position:absolute}.bss-mark i:first-child{background:var(--bss-green);top:2px}.bss-mark i:nth-child(2){top:11px}.bss-mark i:nth-child(3){top:20px}.bss-header nav{border:1px solid var(--bss-line);background:#e9ece6;border-radius:999px;gap:3px;padding:4px;display:flex}.bss-header nav button{color:var(--bss-muted);cursor:pointer;background:0 0;border:0;border-radius:999px;padding:7px 15px;font-size:11px;font-weight:750}.bss-header nav button[aria-current=page]{background:var(--bss-paper);color:var(--bss-ink);box-shadow:0 1px 4px #17211e1f}.bss-status{color:var(--bss-muted);letter-spacing:.08em;text-transform:uppercase;justify-self:end;align-items:center;gap:8px;font-size:10px;font-weight:800;display:flex}.bss-status span{background:#c98236;border-radius:50%;width:8px;height:8px;box-shadow:0 0 0 4px #c982361f}.bss-status.is-ready span{background:#2ca269;box-shadow:0 0 0 4px #2ca2691f}.bss-eyebrow{color:var(--bss-green);letter-spacing:.15em;text-transform:uppercase;font-size:9px;font-weight:900}.bss-button{border:1px solid var(--bss-line);background:var(--bss-paper);color:var(--bss-ink);cursor:pointer;border-radius:7px;justify-content:center;align-items:center;padding:8px 12px;font-size:11px;font-weight:800;display:inline-flex}.bss-button:hover:not(:disabled){background:#f7f7f2;border-color:#9da9a1}.bss-button:disabled{opacity:.5;cursor:not-allowed}.bss-button.primary{border-color:var(--bss-green);background:var(--bss-green);color:#fff}.bss-button.danger{color:#a33e30}.bss-button.danger-solid{color:#fff;background:#b64534;border-color:#b64534}.bss-actions{flex-wrap:wrap;gap:7px;display:flex}.bss-button.file{position:relative;overflow:hidden}.bss-button.file input{opacity:0;cursor:pointer;position:absolute;inset:0}.bss-state{text-align:center;flex-direction:column;justify-content:center;align-items:center;min-height:calc(100vh - 112px);padding:50px 24px;display:flex}.bss-state h1{max-width:740px;margin:9px 0 16px;font:44px/1.05 Georgia,serif}.bss-state p{max-width:620px;color:var(--bss-muted)}.bss-state.error-state>span:first-child{background:var(--bss-coral);color:#fff;border-radius:50%;place-items:center;width:48px;height:48px;font:700 25px Georgia,serif;display:grid}.bss-loader{align-items:end;gap:6px;height:25px;margin-bottom:14px;display:flex}.bss-loader i{background:var(--bss-green);border-radius:50%;width:9px;height:9px;animation:1.1s ease-in-out infinite bss-bounce}.bss-loader i:nth-child(2){animation-delay:.12s}.bss-loader i:nth-child(3){animation-delay:.24s}.bss-setup{max-width:1100px;min-height:calc(100vh - 112px);margin:0 auto;padding:clamp(40px,7vw,82px) 28px}.bss-setup-hero{max-width:760px}.bss-setup-hero h1{letter-spacing:-.025em;margin:10px 0 14px;font:clamp(38px,6vw,64px)/1 Georgia,serif}.bss-setup-hero p{max-width:680px;color:var(--bss-muted);font-size:16px}.bss-setup-grid{grid-template-columns:repeat(3,1fr);gap:13px;margin:38px 0 14px;display:grid}.bss-setup-grid article{border:1px solid var(--bss-line);background:var(--bss-paper);border-radius:12px;min-height:205px;padding:22px}.bss-setup-grid article>span{letter-spacing:.13em;font-size:8px;font-weight:900}.bss-setup-grid .ok{color:var(--bss-green)}.bss-setup-grid .warn{color:#a9621d}.bss-setup-grid .neutral{color:var(--bss-muted)}.bss-setup-grid h2{margin:32px 0 6px;font:20px Georgia,serif}.bss-setup-grid p,.bss-setup-grid small{color:var(--bss-muted);font-size:11px}.bss-setup-grid code{overflow-wrap:anywhere;color:var(--bss-green-dark);font-size:10px;display:block}.bss-callout,.bss-consent{border-radius:12px;margin-top:13px;padding:20px 22px}.bss-callout.is-warning{background:#fff7e8;border:1px solid #e1c596}.bss-callout p{color:#795d32;margin:4px 0 0;font-size:12px}.bss-consent{background:var(--bss-green-soft);border:1px solid #bcd8c7}.bss-consent h2{margin:5px 0;font:23px Georgia,serif}.bss-consent p{color:#446254;max-width:800px;margin:0 0 14px;font-size:12px}.bss-consent label{align-items:center;gap:8px;margin-bottom:13px;font-size:11px;display:flex}.bss-consent>small{min-height:16px;color:var(--bss-green-dark);margin-top:7px;display:block}.setup-error{max-width:1044px;margin:-55px auto 45px}.bss-workspace{grid-template-columns:260px minmax(0,1fr);min-height:calc(100vh - 112px);display:grid}.bss-sidebar{border-right:1px solid var(--bss-line);background:#eaeee8d9}.bss-sidebar>section{border-bottom:1px solid var(--bss-line);padding:21px 17px}.bss-side-title{justify-content:space-between;align-items:center;margin-bottom:13px;display:flex}.bss-side-title h2{letter-spacing:.13em;text-transform:uppercase;margin:0;font-size:10px}.bss-side-title>span{min-width:22px;color:var(--bss-muted);text-align:center;background:#dce2db;border-radius:999px;padding:2px 6px;font-size:9px;font-weight:800}.bss-sidebar details{margin:0 -7px 6px}.bss-sidebar summary{cursor:pointer;border-radius:7px;grid-template-columns:1fr auto;padding:7px;font-size:11px;font-weight:800;display:grid}.bss-sidebar summary::marker{content:\"\"}.bss-sidebar summary span{color:var(--bss-muted)}.bss-sidebar ul{border-left:1px solid #c7cec8;margin:0 0 7px 16px;padding:0 0 0 11px;list-style:none}.bss-sidebar li{flex-direction:column;padding:5px 1px;display:flex}.bss-sidebar li code{font-size:10px}.bss-sidebar li small,.bss-migration small{color:var(--bss-muted);font-size:9px}.bss-migration{align-items:flex-start;gap:10px;display:flex}.bss-migration i,.bss-history i{background:#2ca269;border-radius:50%;flex:none;width:7px;height:7px;margin-top:5px}.bss-migration div{flex-direction:column;min-width:0;display:flex}.bss-migration strong{font:10px SFMono-Regular,monospace}.bss-history{max-height:470px;overflow:auto}.bss-history button{width:100%;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:7px;grid-template-columns:8px minmax(0,1fr);gap:9px;padding:8px 6px;display:grid}.bss-history button:hover{background:#ffffffb3}.bss-history button span{flex-direction:column;min-width:0;display:flex}.bss-history button code{text-overflow:ellipsis;white-space:nowrap;font-size:9px;overflow:hidden}.bss-history button small{color:var(--bss-muted);font-size:8px}.bss-history i.error{background:var(--bss-coral)}.bss-main{min-width:0;padding:25px clamp(18px,4vw,52px) 45px}.bss-boundary{background:var(--bss-green-soft);border:1px solid #b9d7c5;border-radius:10px;grid-template-columns:auto 1fr minmax(160px,330px);align-items:center;gap:12px;max-width:1180px;margin:0 auto 15px;padding:12px 15px;display:grid}.bss-boundary>span{background:var(--bss-green);color:#fff;border-radius:50%;place-items:center;width:28px;height:28px;display:grid}.bss-boundary strong{font-size:11px}.bss-boundary p{color:#466456;margin:1px 0 0;font-size:10px}.bss-boundary code{color:var(--bss-green-dark);text-overflow:ellipsis;white-space:nowrap;background:#fff9;border-radius:5px;padding:5px 7px;font-size:9px;overflow:hidden}.bss-status-grid{grid-template-columns:repeat(3,1fr);gap:10px;max-width:1180px;margin:0 auto 15px;display:grid}.bss-status-grid article{border:1px solid var(--bss-line);background:var(--bss-paper);border-radius:9px;grid-template-columns:1fr auto;gap:2px 10px;padding:14px 16px;display:grid}.bss-status-grid small{color:var(--bss-green);letter-spacing:.12em;grid-column:1/-1;font-size:8px;font-weight:900}.bss-status-grid strong{font:21px Georgia,serif}.bss-status-grid span{color:var(--bss-muted);align-self:end;font-size:9px}.bss-panel,.bss-audit{border:1px solid var(--bss-line);background:var(--bss-paper);border-radius:12px;max-width:1180px;margin:0 auto 15px;overflow:hidden;box-shadow:0 10px 30px #1d29220d}.bss-panel-heading{border-bottom:1px solid var(--bss-line);justify-content:space-between;align-items:center;gap:16px;padding:15px 17px;display:flex}.bss-panel-heading h1,.bss-panel-heading h2{margin:2px 0 0;font:21px Georgia,serif}.bss-panel>textarea{resize:vertical;color:#e2ece6;background:#16221d;border:0;outline:0;width:100%;min-height:220px;padding:19px;font-size:12px;line-height:1.7}.bss-panel>textarea::selection{background:#48b38059}.bss-editor-footer{color:var(--bss-muted);justify-content:space-between;align-items:center;gap:18px;padding:9px 12px 9px 17px;font-size:9px;display:flex}.bss-empty{text-align:center;flex-direction:column;justify-content:center;align-items:center;min-height:190px;display:flex}.bss-empty strong{font:18px Georgia,serif}.bss-empty p{color:var(--bss-muted);font-size:11px}.bss-summary,.bss-limit{color:var(--bss-muted);margin:0;padding:12px 14px;font-size:10px}.bss-result+.bss-result{border-top:1px solid var(--bss-line)}.bss-result-meta{color:var(--bss-muted);letter-spacing:.08em;text-transform:uppercase;background:#f0f2ed;justify-content:space-between;padding:8px 13px;font-size:8px;font-weight:850;display:flex}.bss-table-wrap{max-height:420px;overflow:auto}.bss-table-wrap table{border-collapse:collapse;width:100%;font-size:10px}.bss-table-wrap th{border-bottom:1px solid var(--bss-line);text-align:left;text-transform:uppercase;letter-spacing:.05em;background:#f8f8f4;padding:9px 11px;position:sticky;top:0}.bss-table-wrap td{text-overflow:ellipsis;white-space:nowrap;border-bottom:1px solid #eaede8;max-width:380px;padding:9px 11px;font-family:SFMono-Regular,monospace;overflow:hidden}.bss-table-wrap td.is-null{color:#9aa19d;font-style:italic}.bss-confirm{background:#fff0eb;border:1px solid #d5a194;border-radius:10px;justify-content:space-between;align-items:center;gap:18px;max-width:1180px;margin:0 auto 15px;padding:14px 16px;display:flex}.bss-confirm p{color:#7c5147;margin:3px 0 0;font-size:10px}.bss-confirm code{overflow-wrap:anywhere}.bss-confirm>div:last-child{flex:none;gap:7px;display:flex}.bss-message{border:1px solid;border-radius:9px;max-width:1180px;margin:0 auto 15px;padding:11px 14px;font-size:11px}.bss-message.error{color:#8d392d;background:#fff0ec;border-color:#d8a097;flex-direction:column;display:flex}.bss-message.success{color:#206746;background:#e8f5ed;border-color:#b9d9c6}.bss-confirm.is-repair{background:var(--bss-green-soft);border-color:#b9d7c5}.bss-confirm.is-repair p{color:#466456}.bss-audit ol{margin:0;padding:0;list-style:none}.bss-audit li{border-top:1px solid #eceeea;grid-template-columns:8px 1fr auto;align-items:start;gap:11px;padding:11px 16px;display:grid}.bss-audit li:first-child{border-top:0}.bss-audit li>i{background:#2ca269;border-radius:50%;width:7px;height:7px;margin-top:5px}.bss-audit li>i.error{background:var(--bss-coral)}.bss-audit li strong{font-size:10px}.bss-audit li p{color:var(--bss-muted);margin:2px 0 0;font-size:9px}.bss-audit time{color:var(--bss-muted);font-size:8px}.bss-empty-line{color:var(--bss-muted);margin:0;padding:16px;font-size:10px}.bss-handoff{max-width:960px;min-height:calc(100vh - 112px);margin:0 auto;padding:clamp(50px,8vw,96px) 28px}.bss-handoff>h1{letter-spacing:-.035em;max-width:790px;margin:9px 0 16px;font:clamp(40px,7vw,70px)/.98 Georgia,serif}.bss-handoff>p{max-width:760px;color:var(--bss-muted);font-size:16px;line-height:1.65}.bss-handoff section{background:var(--bss-green-dark);color:#fff;border-radius:12px;margin-top:38px;padding:24px}.bss-handoff section>small{color:#8ed0af;letter-spacing:.13em;font-size:8px;font-weight:900}.bss-handoff section>code{overflow-wrap:anywhere;color:#d5eee0;margin:7px 0 27px;font-size:12px;display:block}.bss-handoff h2{font:21px Georgia,serif}.bss-handoff pre{color:#d9e8df;white-space:pre-wrap;background:#113d2e;border:1px solid #356a54;border-radius:8px;max-height:310px;padding:16px;font-size:10px;line-height:1.65;overflow:auto}.bss-handoff .primary{color:var(--bss-green-dark);background:#fff;border-color:#fff}.backend-sqlite-studio>footer{border-top:1px solid var(--bss-line);background:var(--bss-paper);min-height:40px;color:var(--bss-muted);letter-spacing:.08em;text-transform:uppercase;justify-content:space-between;align-items:center;gap:18px;padding:10px 28px;font-size:8px;display:flex}@keyframes bss-bounce{0%,60%,to{opacity:.45;transform:translateY(0)}30%{opacity:1;transform:translateY(-9px)}}@media (width<=850px){.bss-header{grid-template-columns:1fr auto}.bss-header nav{order:3;grid-column:1/-1;justify-self:center}.bss-status{display:none}.bss-setup-grid{grid-template-columns:1fr}.bss-setup-grid article{min-height:150px}.bss-workspace{grid-template-columns:1fr}.bss-sidebar{border-right:0;border-bottom:1px solid var(--bss-line);grid-template-columns:repeat(3,1fr);display:grid}.bss-sidebar>section{border-right:1px solid var(--bss-line)}.bss-boundary{grid-template-columns:auto 1fr}.bss-boundary code{grid-column:1/-1;max-width:100%}.bss-audit time{display:none}}@media (width<=600px){.bss-header{padding:12px 15px}.bss-main{padding:16px 11px 35px}.bss-sidebar{display:block}.bss-history{display:none}.bss-status-grid{grid-template-columns:1fr}.bss-panel-heading{flex-direction:column;align-items:flex-start}.bss-editor-footer,.bss-confirm{flex-direction:column;align-items:stretch}.bss-confirm>div:last-child{justify-content:flex-end}.bss-handoff,.bss-setup{padding-left:18px;padding-right:18px}.backend-sqlite-studio>footer{flex-direction:column;align-items:flex-start}}@media (prefers-reduced-motion:reduce){.bss-loader i{animation:none}}", Ae = ["studio", "handoff"];
function je(e) {
	if (e.apiVersion !== "1") throw Error(`Backend SQLite Studio requires Canvas host API 1, received ${e.apiVersion}.`);
	let t = Ae.map((t) => e.registerPage(t, ({ container: n, path: r, navigate: i }) => {
		let a = new AbortController(), o = document.createElement("style");
		o.dataset.backendSqliteStudio = "styles", o.textContent = ke;
		let s = document.createElement("div");
		s.dataset.backendSqliteStudio = "root", n.append(o, s);
		let c = (0, u.createRoot)(s);
		return c.render(/* @__PURE__ */ (0, k.jsx)(Oe, {
			host: e,
			pageId: t,
			path: r,
			navigate: i,
			...
			a.abort(), c.unmount(), s.remove(), o.remove();
		};
	}));
	return () => {
		for (let e of t) e();
...
var Me = { PAGES: Ae };
//#endregion
export { Me as __testing, je as activate };
