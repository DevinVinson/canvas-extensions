...
function makeHost(options: Options = {}) {
  const mounts = new Map<string, (context: MountContext) => void | (() => void)>(); const unregister = vi.fn(); const requests: AgentServerRequest[] = [];
  ...
  } }, registerPage: vi.fn((id, mount) => { mounts.set(id, mount); return unregister; }), navigate: vi.fn() };
  return { host, mounts, unregister, requests };
}
...
async function mountPage(path = "", options: Options = {}) { const fixture = makeHost(options); const deactivate = activate(fixture.host); const container = document.createElement("div"); const navigate = vi.fn(); let cleanup: void | (() => void); await act(async () => { cleanup = fixture.mounts.get("search")!({ container, path, navigate }); }); await flush(); return { ...fixture, container, navigate, cleanup: cleanup!, deactivate }; }
...
describe("activation, routes, and cleanup", () => {
  it("registers exactly one manifest page and unregisters it", () => { const fixture = makeHost(); const deactivate = activate(fixture.host); expect(manifest.contributes.pages.map((page) => page.id)).toEqual(["search"]); expect(fixture.host.registerPage).toHaveBeenCalledWith("search", expect.any(Function)); expect(fixture.host.registerPage).toHaveBeenCalledTimes(1); deactivate(); expect(fixture.unregister).toHaveBeenCalledOnce(); });
  it.each([["", "search"], ["search/details", "search"], ["operations", "operations"], ["operations/logs", "operations"], ["other", "not-found"]])("normalizes nested route %s", (path, view) => expect(normalizeRoute(path)).toBe(view));
  it("uses mount navigation and removes DOM on cleanup", async () => { const mounted = await mountPage("other"); button(mounted.container, "Open search").click(); expect(mounted.navigate).toHaveBeenCalledWith(SEARCH_ROUTE); await act(async () => mounted.cleanup()); expect(mounted.container.childElementCount).toBe(0); mounted.deactivate(); });
});

describe("onboarding and search", () => {
  it("shows read-only diagnostics and gates local installation behind consent", async () => { const mounted = await mountPage("", { installed: false }); expect(mounted.container.textContent).toContain("read-only probe"); expect(mounted.container.textContent).toContain(`${HOME}/.openhands/dev_conversations`); expect(mounted.container.textContent).toContain("Sensitive local data"); const install = button(mounted.container, "Build & install"); expect(install.disabled).toBe(true); await act(async () => (mounted.container.querySelector('input[type="checkbox"]') as HTMLInputElement).click()); expect(install.disabled).toBe(false); await act(async () => install.click()); await flush(); expect(mounted.requests.some((request) => (request.body as { command?: string } | undefined)?.command === INSTALL_COMMAND)).toBe(true); await act(async () => mounted.cleanup()); mounted.deactivate(); });
  it("offers an agent prompt when Go is missing", async () => { const mounted = await mountPage("", { installed: false, go: false }); expect(mounted.container.textContent).toContain("Go 1.23+ was not found"); await act(async () => button(mounted.container, "Copy agent setup prompt").click()); expect(navigator.clipboard.writeText).toHaveBeenCalled(); await act(async () => mounted.cleanup()); mounted.deactivate(); });
...
describe("index and service lifecycle", () => {
  ...
  it("navigates between nested views on the single registered page", async () => { const search = await mountPage(); button(search.container, "Index operations").click(); expect(search.navigate).toHaveBeenCalledWith(OPERATIONS_ROUTE); expect(OPERATIONS_ROUTE).toBe(`${SEARCH_ROUTE}/operations`); await act(async () => search.cleanup()); search.deactivate(); const operations = await mountPage("operations"); button(operations.container, "Search").click(); expect(operations.navigate).toHaveBeenCalledWith(SEARCH_ROUTE); await act(async () => operations.cleanup()); operations.deactivate(); });
});
