...
var e = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), t = /* @__PURE__ */ e(((e) => {
	...
	function Wa(e) {
		...
		function l(e, t, n, r) {
			var a = n.type;
			return a === y ? d(e, t, n.props.children, r, n.key) : t !== null && (t.elementType === a || typeof a == "object" && a && a.$$typeof === ie && Fa(a) === t.type) ? (t = i(t, n.props), Ha(t, n), t.return = e, t) : (t = xi(n.type, n.key, n.props, null, e.mode, r), Ha(t, n), t.return = e, t);
		}
		function u(e, t, n, r) {
	...
	function Ic(e, t, n) {
		...
		switch (t.lanes = 0, t.tag) {
			...
			case 27: return Se(t), e === null && j && (r = t.stateNode = ff(t.type, t.pendingProps, ve.current), Hi = t, Wi = !0, i = A, Zd(t.type) ? (lf = i, A = cf(r.firstChild)) : A = i), cc(e, t, t.pendingProps.children, n), vc(e, t), e === null && (t.flags |= 4194304), t.child;
			case 5: return e === null && j && ((i = r = A) && (r = tf(r, t.type, t.pendingProps, Wi), r === null ? i = !1 : (t.stateNode = r, Hi = t, A = cf(r.firstChild), Wi = !1, i = !0)), i || Ki(t)), Se(t), i = t.type, a = t.pendingProps, o = e === null ? null : e.memoizedProps, r = a.children, Ud(i, a) ? r = null : o !== null && Ud(i, o) && (t.flags |= 32), t.memoizedState !== null && (i = Oo(e, t, jo, null, null, n), Qf._currentValue = i), vc(e, t), cc(e, t, r, n), t.child;
	...
	function Hc(e, t, n) {
		...
		switch (Bi(t), t.tag) {
			...
			case 19:
				...
				return r.tail === null ? (B(t), null) : (e = r.tail, r.rendering = e, r.tail = e.sibling, r.renderingStartTime = Fe(), e.sibling = null, n = N.current, D(N, i ? n & 1 | 2 : n & 1), j && Li(t, r.treeForkCount), e);
			case 22:
	...
	source_sha256: "9a126991a6a6cae09f441398d3a0ebb17365d2f92e955d14ca24088e76b8cfaa",
	main_go_sha256: "cab411491c1056c7ec26c71e47d98daaa8f1b5b4393ba218a0f4a10c9424e621",
	go_mod_sha256: "4386d0e31d7d23f7a6bf2b2e87a824e7f0fa45ddc814144774f3e64a38389ce2",
	go_sum_sha256: "7e02c5aa2fbfe6d1567a36b9bff80c5b6478f16c1a7e5a1050e4fe67d946153e",
	origin: "Bundled App source in native/main.go, native/go.mod, and native/go.sum",
	module_network: "The first local build downloads the pinned Bleve module graph from proxy.golang.org or the configured Go proxy."
}, h = ".openhands/apps/conversation-search-sidecar", g = m.version, _ = m.source_sha256;
function v(e) {
...
var b = v(d), x = v(f), ee = v(p), S = v(JSON.stringify({ action: "stop" })), C = v(JSON.stringify({ action: "status" })), te = String.raw`python3 - <<'PY'
...
if paths_safe and binary.is_file() and os.access(binary, os.X_OK) and artifact.is_file() and runtime_version == "${"1"}":
    try:
        metadata = json.loads(artifact.read_text())
        digest = hashlib.sha256()
        with binary.open("rb") as stream:
            for chunk in iter(lambda: stream.read(1024 * 1024), b""): digest.update(chunk)
        artifact_verified = metadata.get("version") == "${g}" and metadata.get("sourceSha256") == "${_}" and metadata.get("binarySha256") == digest.hexdigest()
        if artifact_verified:
            ...
            installed = result.returncode == 0 and envelope.get("ok") is True and envelope.get("data", {}).get("message", "").startswith("conversation-search ${g} ")
...
decode_file '${b}' "$build_dir/main.go"
decode_file '${x}' "$build_dir/go.mod"
decode_file '${ee}' "$build_dir/go.sum"
[ "$(sha_file "$build_dir/main.go")" = '${m.main_go_sha256}' ] || { printf 'bundled main.go checksum mismatch\n' >&2; exit 1; }
[ "$(sha_file "$build_dir/go.mod")" = '${m.go_mod_sha256}' ] || { printf 'bundled go.mod checksum mismatch\n' >&2; exit 1; }
[ "$(sha_file "$build_dir/go.sum")" = '${m.go_sum_sha256}' ] || { printf 'bundled go.sum checksum mismatch\n' >&2; exit 1; }
...
[ "$artifact_version" = '${g}' ] && [ "$artifact_source" = '${_}' ] && [ "$(cat "$app_dir/.runtime-version" 2>/dev/null)" = '${"1"}' ] || { printf 'native artifact version/source mismatch; use Repair runtime\n' >&2; exit 1; }
...
if "$binary" rpc '${C}' >/dev/null 2>&1; then
...
while [ "$attempt" -lt 50 ]; do
  if "$binary" rpc '${C}' >/dev/null 2>&1; then printf 'CONVERSATION_SEARCH_STARTED\tready\n'; exit 0; fi
  attempt=$((attempt + 1))
  sleep 0.1
done
printf 'native service did not become healthy; inspect the App log\n' >&2
exit 1`, ie = String.raw`set -eu
app_dir='.openhands/apps/conversation-search-sidecar'
binary="$app_dir/bin/conversation-search"
...
[ "$artifact_version" = '${g}' ] && [ "$artifact_source" = '${_}' ] && [ "$(cat "$app_dir/.runtime-version" 2>/dev/null)" = '${"1"}' ] && [ "$(sha_file "$binary")" = "$expected" ] || { printf 'native artifact verification failed; use Repair runtime\n' >&2; exit 1; }
if "$binary" rpc '${S}' >/dev/null 2>&1; then printf 'CONVERSATION_SEARCH_STOPPED\tstopped\n'
else printf 'CONVERSATION_SEARCH_STOPPED\talready-stopped\n'; fi`, ae = String.raw`set -eu
app_dir='.openhands/apps/conversation-search-sidecar'
binary="$app_dir/bin/conversation-search"
artifact="$app_dir/artifact.json"
...
if [ -x "$binary" ]; then
  ...
  artifact_source=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["sourceSha256"])' "$artifact")
  expected=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["binarySha256"])' "$artifact")
  [ "$artifact_version" = '${g}' ] && [ "$artifact_source" = '${_}' ] && [ "$(cat "$app_dir/.runtime-version" 2>/dev/null)" = '${"1"}' ] && [ "$(sha_file "$binary")" = "$expected" ] || { printf 'native artifact verification failed; repair and stop before deletion\n' >&2; exit 1; }
  "$binary" rpc '${S}' >/dev/null 2>&1 || true
fi
...
function ce(e) {
	if (!se(e)) throw Error("The Agent Server returned an unsafe home path.");
	return `${e.replace(/\/+$/, "")}/${h}`;
}
async function le(e, t) {
	...
	if (!se(r?.home)) throw Error("The Agent Server did not report a safe absolute home directory.");
	return r.home;
}
async function ue(e, t, n, r, i) {
	if (!se(t)) throw Error("The Agent Server home path failed validation.");
	oe(i);
	...
	oe(i);
	let o = typeof a?.stdout == "string" ? a.stdout : "", s = typeof a?.stderr == "string" ? a.stderr.trim() : "";
	if ((typeof a?.exit_code == "number" ? a.exit_code : -1) !== 0) throw Error(s || "The Agent Server command failed.");
	return o;
}
function de(e, t) {
	let n = e.split(/\r?\n/).find((e) => e.startsWith(`${t}\t`));
	if (!n) throw Error(`The native bridge did not return ${t}.`);
	return n.slice(t.length + 1).trim();
}
async function w(e, t, n) {
	let r = de(await ue(e, t, te, 20, n), "CONVERSATION_SEARCH_PROBE"), i;
	try {
		i = JSON.parse(y(r));
	} catch {
		throw Error("The prerequisite probe returned malformed data.");
	}
	if (typeof i.supported != "boolean" || typeof i.goReady != "boolean" || typeof i.installed != "boolean" || typeof i.artifactVerified != "boolean" || typeof i.target != "string") throw Error("The prerequisite probe was incomplete.");
	return i;
}
function T(e) {
	let t = de(e, "CONVERSATION_SEARCH"), n;
	try {
		n = JSON.parse(y(t));
	} catch {
		throw Error("The native bridge returned malformed data.");
	}
	if (n.ok !== !0) throw Error(typeof n.error == "string" ? n.error : "The native bridge reported an unknown error.");
	if (!n.data || typeof n.data != "object") throw Error("The native bridge returned no structured result.");
	return n.data;
}
function fe(e, t = "once") {
	let n = v(JSON.stringify(e));
...
async function pe(e, t, n) {
	T(await ue(e, t, ne, 900, n));
}
async function me(e, t, n) {
	await ue(e, t, re, 30, n);
}
async function he(e, t, n) {
	await ue(e, t, ie, 20, n);
}
async function E(e, t, n) {
	try {
		await he(e, t, n);
	} catch (e) {
		if (!(e instanceof Error && /artifact|binary checksum|runtime is missing/.test(e.message))) throw e;
	}
	await pe(e, t, n), await he(e, t, n);
}
async function D(e, t, n) {
	await ue(e, t, ae, 30, n);
}
async function ge(e, t, n, r, i) {
	let a = "action" in n ? String(n.action) : "", o = [
		"index",
		"rebuild",
		"repair"
	].includes(a) ? 900 : 60;
	return T(await ue(e, t, fe(n, r), o, i));
}
async function _e(e, t, n, r) {
	let i = await ge(e, t, { action: "status" }, n, r);
	if (!i.status) throw Error("The native bridge omitted index status.");
	return i.status;
}
async function ve(e, t, n, r, i) {
	let a = await ge(e, t, { action: n ? "rebuild" : "index" }, r, i);
	if (!a.status) throw Error("The native bridge omitted index status.");
	return a.status;
}
async function ye(e, t, n, r, i, a) {
	...
	if (!o.search) throw Error("The native bridge omitted search results.");
	return o.search;
}
async function be(e, t, n, r, i) {
	...
	if (!a.hit) throw Error("The native bridge omitted the indexed event.");
	return a.hit;
}
function xe(e) {
	let t = ce(e);
	return `Set up Conversation Search Sidecar on this Agent Server. First inspect the host OS/architecture, Go availability, the App's bundled source under its installed copy, and ${t}. Explain every package/network/filesystem change and obtain my approval before installing anything. Build only the pinned Go module from the App's reviewed source; verify its source checksums from artifact-metadata.json; place all build caches, binary, logs, run state, source manifest, and Bleve index below ${t}; record and verify the resulting binary SHA-256; do not read or change conversation source data during setup; do not expose a general shell or daemon port; then tell me to use Recheck in the App.`;
}
//#endregion
...
var Se = /* @__PURE__ */ e(((e) => {
	...
	t.exports = Se();
})))(), Ce = "/extensions/conversation-search-sidecar/search", we = `${Ce}/operations`;
function Te(e) {
	...
	return /* @__PURE__ */ (0, O.jsxs)("header", {
		...
		children: [
			/* @__PURE__ */ (0, O.jsxs)("button", {
				...
				}), /* @__PURE__ */ (0, O.jsxs)("span", { children: [/* @__PURE__ */ (0, O.jsx)("small", { children: "LOCAL NATIVE INDEX" }), /* @__PURE__ */ (0, O.jsx)("strong", { children: "Conversation Search" })] })]
			}),
			/* @__PURE__ */ (0, O.jsxs)("nav", {
				...
				children: [/* @__PURE__ */ (0, O.jsx)("button", {
					...
					type: "button",
					"aria-current": e === "operations" ? "page" : void 0,
					onClick: () => r(we),
					children: "Index operations"
				})]
			}),
			/* @__PURE__ */ (0, O.jsxs)("div", {
				className: `css-runtime ${t ? "ready" : ""}`,
				children: [/* @__PURE__ */ (0, O.jsx)("span", {}), t ? n ? "Service active" : "Command mode" : "Setup needed"]
			})
		]
	});
}
function Ne({ location: e }) {
	return /* @__PURE__ */ (0, O.jsxs)("article", {
		className: "css-location",
		children: [
			/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("span", {
				className: `css-pill ${e.state}`,
				children: e.state
			}), /* @__PURE__ */ (0, O.jsx)("small", { children: e.layout })] }),
			/* @__PURE__ */ (0, O.jsx)("code", { children: e.path }),
			/* @__PURE__ */ (0, O.jsx)("p", { children: e.state === "ready" ? `${e.conversations.toLocaleString()} conversations · ${e.files.toLocaleString()} indexable files` : e.message ?? "No conversation data detected here." })
		]
	});
}
function Pe({ home: e, probe: t, busy: n, error: r, onInstall: i, onRecheck: a }) {
	let [o, s] = (0, l.useState)(!1), [c, u] = (0, l.useState)(!1), d = t.supported && t.goReady, f = async () => {
		await navigator.clipboard.writeText(xe(e)), u(!0);
	};
			...
			/* @__PURE__ */ (0, O.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, O.jsxs)("div", { children: [
					/* @__PURE__ */ (0, O.jsx)("span", {
						...
						children: "PRIVATE BY CONSTRUCTION"
					}),
					/* @__PURE__ */ (0, O.jsx)("h1", { children: "Search years of agent work in milliseconds." }),
					/* @__PURE__ */ (0, O.jsx)("p", { children: "Conversation text stays on this Agent Server. The read-only probe below did not create a directory, compile code, or open a conversation file." })
				] }), /* @__PURE__ */ (0, O.jsxs)("div", {
					className: "css-orbit",
					"aria-hidden": "true",
					children: [
						...
						/* @__PURE__ */ (0, O.jsx)("span", {}),
						/* @__PURE__ */ (0, O.jsx)("span", {}),
						/* @__PURE__ */ (0, O.jsx)("b", { children: "⌕" })
					]
				})]
			}),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				className: "css-setup-grid",
				children: [
					/* @__PURE__ */ (0, O.jsxs)("article", { children: [
						/* @__PURE__ */ (0, O.jsx)("span", {
							className: t.supported ? "ok" : "bad",
							children: t.supported ? "SUPPORTED" : "UNSUPPORTED"
						}),
						/* @__PURE__ */ (0, O.jsx)("h2", { children: t.target }),
						/* @__PURE__ */ (0, O.jsxs)("p", { children: [
							t.os,
							" · ",
							t.arch
						] }),
						/* @__PURE__ */ (0, O.jsx)("small", { children: "Supported: macOS and Linux on arm64/amd64." })
					] }),
					/* @__PURE__ */ (0, O.jsxs)("article", { children: [
						/* @__PURE__ */ (0, O.jsx)("span", {
							className: t.goReady ? "ok" : "warn",
							children: t.goReady ? "READY" : "MISSING / OLD"
						}),
						/* @__PURE__ */ (0, O.jsx)("h2", { children: "Local Go build" }),
						/* @__PURE__ */ (0, O.jsx)("p", { children: t.goVersion ?? "Go 1.23+ was not found." }),
						/* @__PURE__ */ (0, O.jsx)("small", { children: "Go 1.23+ is required; no unverified prebuilt binary is executed." })
					] }),
					/* @__PURE__ */ (0, O.jsxs)("article", { children: [
						/* @__PURE__ */ (0, O.jsx)("span", {
							className: "neutral",
						...
						/* @__PURE__ */ (0, O.jsx)("h2", { children: "Durable local state" }),
						/* @__PURE__ */ (0, O.jsx)("code", { children: ce(e) }),
						/* @__PURE__ */ (0, O.jsx)("small", { children: "Source, module/build cache, binary, index, logs, and run state live only here." })
					] })
				]
			}),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, O.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("span", {
						className: "css-eyebrow",
						children: "DATA LOCATION DIAGNOSTICS"
					}), /* @__PURE__ */ (0, O.jsx)("h2", { children: "Candidate SDK stores" })] }), /* @__PURE__ */ (0, O.jsx)("button", {
						className: "css-button quiet",
						type: "button",
						onClick: a,
						disabled: !!n,
						children: "Recheck"
					...
					children: t.locations.map((e) => /* @__PURE__ */ (0, O.jsx)(Ne, { location: e }, e.path))
				})]
			}),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				className: "css-trust",
				children: [
					/* @__PURE__ */ (0, O.jsxs)("div", { children: [
						/* @__PURE__ */ (0, O.jsx)("span", {
							className: "css-eyebrow",
							children: "DELIBERATE INSTALL"
						}),
						/* @__PURE__ */ (0, O.jsxs)("h2", { children: ["Build Conversation Search ", g] }),
						/* @__PURE__ */ (0, O.jsxs)("p", { children: [
							"The App writes its reviewed Go source, verifies SHA-256 ",
							/* @__PURE__ */ (0, O.jsxs)("code", { children: [_.slice(0, 16), "…"] }),
							", downloads the pinned Bleve module graph using the configured Go proxy, compiles locally for ",
							/* @__PURE__ */ (0, O.jsx)("strong", { children: t.target }),
							", records the binary SHA-256, and verifies the version and hash before every native execution."
						] }),
						/* @__PURE__ */ (0, O.jsxs)("p", {
							className: "css-privacy",
							children: [/* @__PURE__ */ (0, O.jsx)("strong", { children: "Sensitive local data:" }), " indexing creates a searchable copy of message text, selected command/output text, titles, timestamps, roles, and tool summaries. Hidden reasoning, system prompts, base state, and configured secrets are excluded. Nothing is uploaded by the App."]
						})
					] }),
					/* @__PURE__ */ (0, O.jsxs)("label", {
						...
						}), " I understand the disk/network actions and local sensitive-data copy."]
					}),
					d ? null : /* @__PURE__ */ (0, O.jsx)("div", {
						className: "css-warning",
						role: "alert",
						children: "This target needs a supported platform and Go 1.23+. Use the copyable agent prompt to inspect and install prerequisites with your approval."
					}),
					...
					/* @__PURE__ */ (0, O.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, O.jsx)("button", {
							...
							disabled: !o || !d || !!n,
							onClick: i,
							children: n || "Build & install locally"
						}), /* @__PURE__ */ (0, O.jsx)("button", {
							className: "css-button",
							type: "button",
							onClick: () => void f(),
							children: c ? "Prompt copied" : "Copy agent setup prompt"
						})]
					})
				]
			})
...
function Fe({ status: e }) {
	return /* @__PURE__ */ (0, O.jsxs)("section", {
		...
		children: [
			/* @__PURE__ */ (0, O.jsxs)("article", { children: [
				/* @__PURE__ */ (0, O.jsx)("small", { children: "DOCUMENTS" }),
				/* @__PURE__ */ (0, O.jsx)("strong", { children: e.documents.toLocaleString() }),
				/* @__PURE__ */ (0, O.jsxs)("span", { children: [e.sourceFiles.toLocaleString(), " source files"] })
			] }),
			/* @__PURE__ */ (0, O.jsxs)("article", { children: [
				/* @__PURE__ */ (0, O.jsx)("small", { children: "CONVERSATIONS" }),
				/* @__PURE__ */ (0, O.jsx)("strong", { children: e.conversations.toLocaleString() }),
				/* @__PURE__ */ (0, O.jsxs)("span", { children: [e.malformedFiles, " malformed skipped"] })
			] }),
			/* @__PURE__ */ (0, O.jsxs)("article", { children: [
				/* @__PURE__ */ (0, O.jsx)("small", { children: "INDEX SIZE" }),
				/* @__PURE__ */ (0, O.jsx)("strong", { children: Oe(e.indexBytes) }),
				/* @__PURE__ */ (0, O.jsx)("span", { children: e.lastIndexedAt ? `Updated ${new Date(e.lastIndexedAt).toLocaleString()}` : "Not indexed yet" })
			] }),
			/* @__PURE__ */ (0, O.jsxs)("article", { children: [
				/* @__PURE__ */ (0, O.jsx)("small", { children: "EXECUTION" }),
				/* @__PURE__ */ (0, O.jsx)("strong", { children: e.service.running ? "Service" : "Command" }),
				/* @__PURE__ */ (0, O.jsx)("span", { children: e.service.running ? `Idle stop in ${Math.max(0, 600 - (e.service.idleSeconds ?? 0))}s` : "One-shot CLI bridge" })
			] })
		]
	});
}
	...
	return /* @__PURE__ */ (0, O.jsxs)("aside", {
		...
		children: [
			...
			/* @__PURE__ */ (0, O.jsxs)("dl", { children: [
				...
				/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("dt", { children: "Kind / role" }), /* @__PURE__ */ (0, O.jsxs)("dd", { children: [
					t.kind,
					" · ",
					t.role
				] })] }),
				/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("dt", { children: "Tool" }), /* @__PURE__ */ (0, O.jsx)("dd", { children: t.tool || "—" })] }),
				/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("dt", { children: "Timestamp" }), /* @__PURE__ */ (0, O.jsx)("dd", { children: t.timestamp || "—" })] }),
				/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("dt", { children: "Source" }), /* @__PURE__ */ (0, O.jsx)("dd", { children: /* @__PURE__ */ (0, O.jsx)("code", { children: t.sourcePath }) })] })
			] }),
			/* @__PURE__ */ (0, O.jsx)("h3", { children: "Indexed text" }),
			/* @__PURE__ */ (0, O.jsx)("pre", { children: t.text || t.summary || "This record contains metadata only." }),
			/* @__PURE__ */ (0, O.jsx)("p", {
				className: "css-muted",
				children: "This is the selected, locally indexed representation—not the raw source JSON."
			})
		]
	});
}
function Le({ host: e, home: t, status: n, setStatus: r, signal: i }) {
	...
	return /* @__PURE__ */ (0, O.jsxs)("main", {
		...
		children: [
			/* @__PURE__ */ (0, O.jsx)(Fe, { status: n }),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				className: "css-search-panel",
				children: [
					/* @__PURE__ */ (0, O.jsxs)("div", {
						className: "css-section-title",
						children: [/* @__PURE__ */ (0, O.jsxs)("div", { children: [/* @__PURE__ */ (0, O.jsx)("span", {
				...
				children: [
					...
					u?.hits.map((t) => /* @__PURE__ */ (0, O.jsxs)("article", {
						...
						children: [
							/* @__PURE__ */ (0, O.jsxs)("div", { children: [
								/* @__PURE__ */ (0, O.jsx)("span", {
									className: "css-role",
									children: t.role || "unknown"
								}),
								/* @__PURE__ */ (0, O.jsx)("span", { children: t.kind }),
								t.tool ? /* @__PURE__ */ (0, O.jsx)("span", { children: t.tool }) : null,
								/* @__PURE__ */ (0, O.jsx)("time", { children: t.timestamp ? new Date(t.timestamp).toLocaleString() : "No timestamp" })
							] }),
							/* @__PURE__ */ (0, O.jsx)("h2", { children: /* @__PURE__ */ (0, O.jsx)(je, {
								host: e,
								conversationId: t.conversationId,
					...
					u && u.hits.length === 0 ? /* @__PURE__ */ (0, O.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, O.jsx)("h2", { children: "No indexed events matched." }),
							/* @__PURE__ */ (0, O.jsx)("p", { children: "Try fewer filters, a broader phrase, or refresh the index." })
						]
					}) : null
				]
			}),
...
function Re({ host: e, home: t, status: n, setStatus: r, recheck: i, signal: a }) {
	let [o, s] = (0, l.useState)(""), [c, u] = (0, l.useState)(""), [d, f] = (0, l.useState)(""), [p, m] = (0, l.useState)(!1), h = n.service.running ? "rpc" : "once", g = async (e, t) => {
		s(e), u(""), f("");
		try {
			await t(), f(`${e} completed.`);
		} catch (e) {
			De(e) || u(Ee(e));
		} finally {
			s("");
		}
	}, _ = () => g("Recheck", async () => r(await _e(e, t, n.service.running ? "rpc" : "once", a))), v = (n) => {
		(!n || window.confirm("Rebuild deletes only the App-owned Bleve index and source manifest, then recreates them from read-only conversation data. Continue?")) && g(n ? "Full rebuild" : "Incremental index", async () => r(await ve(e, t, n, h, a)));
	}, y = () => void g(n.service.running ? "Stop service" : "Start service", async () => {
		n.service.running ? await he(e, t, a) : await me(e, t, a), r(await _e(e, t, "once", a));
	}), b = () => {
		window.confirm("Repair runtime stops the service, rewrites the reviewed source, verifies checksums, and recompiles the binary. The index is preserved. Continue?") && g("Repair runtime", async () => {
			await E(e, t, a), i();
		});
	}, x = () => void g("Delete App data", async () => {
		await D(e, t, a), i();
	});
	return /* @__PURE__ */ (0, O.jsxs)("main", {
		className: "css-operations",
		children: [
			/* @__PURE__ */ (0, O.jsx)(Fe, { status: n }),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				className: "css-section css-operation-hero",
				children: [/* @__PURE__ */ (0, O.jsxs)("div", { children: [
					/* @__PURE__ */ (0, O.jsx)("span", {
						className: "css-eyebrow",
						children: "CONTROL PLANE"
					}),
					/* @__PURE__ */ (0, O.jsx)("h1", { children: "Own the index lifecycle." }),
					/* @__PURE__ */ (0, O.jsxs)("p", { children: [
						"Conversation sources are always read-only. Every mutation stays below ",
						/* @__PURE__ */ (0, O.jsx)("code", { children: ce(t) }),
						"."
					] })
				] }), /* @__PURE__ */ (0, O.jsx)("button", {
					className: "css-button",
			...
			d ? /* @__PURE__ */ (0, O.jsx)("div", {
				className: "css-notice",
				role: "status",
				children: d
			}) : null,
			/* @__PURE__ */ (0, O.jsxs)("section", {
				className: "css-operation-grid",
				children: [
					/* @__PURE__ */ (0, O.jsxs)("article", { children: [
						/* @__PURE__ */ (0, O.jsx)("span", {
							className: "css-step",
						...
						/* @__PURE__ */ (0, O.jsx)("h2", { children: "Index content" }),
						/* @__PURE__ */ (0, O.jsx)("p", { children: "Incremental updates compare source path, size, nanosecond mtime, and SHA-256 fallback. Changed records are upserted and deleted sources are removed." }),
						/* @__PURE__ */ (0, O.jsxs)("div", {
							className: "css-actions",
							children: [/* @__PURE__ */ (0, O.jsx)("button", {
								className: "css-button primary",
								onClick: () => v(!1),
								disabled: !!o,
								children: o === "Incremental index" ? "Indexing…" : "Index now"
							}), /* @__PURE__ */ (0, O.jsx)("button", {
								className: "css-button",
								onClick: () => v(!0),
								disabled: !!o,
						...
						n.added || n.updated || n.removed || n.skipped ? /* @__PURE__ */ (0, O.jsxs)("small", { children: [
							"Last run: ",
							n.added ?? 0,
							" added · ",
							n.updated ?? 0,
					...
					/* @__PURE__ */ (0, O.jsxs)("article", { children: [
						...
						/* @__PURE__ */ (0, O.jsx)("h2", { children: "Native service" }),
						/* @__PURE__ */ (0, O.jsx)("p", { children: "The browser never calls localhost. Bash invokes the verified companion CLI, which reads an App-private token/port state file and performs JSON RPC." }),
						/* @__PURE__ */ (0, O.jsx)("div", {
							className: "css-actions",
							children: /* @__PURE__ */ (0, O.jsx)("button", {
								className: "css-button primary",
								onClick: y,
								disabled: !!o,
								children: n.service.running ? "Stop service" : "Start service"
							})
						}),
						/* @__PURE__ */ (0, O.jsxs)("small", { children: [
							"Automatic idle shutdown after ",
							10,
							" minutes; App disablement leaves no permanent daemon."
						] })
					] }),
					/* @__PURE__ */ (0, O.jsxs)("article", { children: [
						/* @__PURE__ */ (0, O.jsx)("span", {
						...
						/* @__PURE__ */ (0, O.jsx)("h2", { children: "Recover safely" }),
						/* @__PURE__ */ (0, O.jsx)("p", { children: "Repair runtime recompiles from verified source and preserves the index. Full rebuild repairs an unreadable index from unchanged source data." }),
						/* @__PURE__ */ (0, O.jsx)("button", {
							className: "css-button",
							onClick: b,
							disabled: !!o,
							children: "Repair runtime"
						})
					] })
				]
			}),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, O.jsx)("div", {
					...
					}), /* @__PURE__ */ (0, O.jsx)("h2", { children: "Validated locations" })] })
				}), /* @__PURE__ */ (0, O.jsx)("div", {
					className: "css-locations",
					children: n.locations.map((e) => /* @__PURE__ */ (0, O.jsx)(Ne, { location: e }, e.path))
				})]
			}),
			/* @__PURE__ */ (0, O.jsxs)("section", {
				className: "css-danger",
				children: [/* @__PURE__ */ (0, O.jsxs)("div", { children: [
					/* @__PURE__ */ (0, O.jsx)("h2", { children: "Uninstall App-owned data" }),
					/* @__PURE__ */ (0, O.jsx)("p", { children: "Stop the service and permanently delete the binary, local source/build cache, Bleve index, manifests, logs, and run state. Conversation sources and sibling Apps are untouched." }),
					/* @__PURE__ */ (0, O.jsx)("code", { children: ce(t) })
					...
					className: "css-button danger",
					onClick: () => m(!0),
					children: "Review deletion"
				})]
			})
		]
	});
}
function ze({ host: e, path: t, navigate: n, signal: r }) {
	let i = Te(t), [a, o] = (0, l.useState)(0), [s, c] = (0, l.useState)(""), [u, d] = (0, l.useState)(null), [f, p] = (0, l.useState)(null), [m, h] = (0, l.useState)(""), [g, _] = (0, l.useState)("");
	(0, l.useEffect)(() => {
		c(""), d(null), p(null), _(""), le(e, r).then(async (t) => {
			c(t);
			let n = await w(e, t, r);
			d(n), n.installed && p(await _e(e, t, "once", r));
		}).catch((e) => {
			De(e) || _(Ee(e));
		});
	}, [
	...
	let v = (0, l.useCallback)(() => o((e) => e + 1), []), y = (0, l.useCallback)(async () => {
		...
		e,
		r
	]), b = (0, l.useMemo)(() => i === "not-found" ? /* @__PURE__ */ (0, O.jsxs)("main", {
		className: "css-state",
		children: [
			/* @__PURE__ */ (0, O.jsx)("span", {
				className: "css-eyebrow",
			...
			/* @__PURE__ */ (0, O.jsx)("h1", { children: "This page is not part of Conversation Search." }),
			/* @__PURE__ */ (0, O.jsx)("button", {
				className: "css-button primary",
				onClick: () => n(Ce),
		...
		children: [
			...
			/* @__PURE__ */ (0, O.jsx)("h1", { children: "Could not inspect this Agent Server." }),
			/* @__PURE__ */ (0, O.jsx)("p", { children: g }),
			/* @__PURE__ */ (0, O.jsx)("button", {
				className: "css-button primary",
				onClick: v,
				children: "Try again"
		...
		children: [
			...
			/* @__PURE__ */ (0, O.jsx)("span", {
				className: "css-eyebrow",
				children: "READ-ONLY PROBE"
			}),
			/* @__PURE__ */ (0, O.jsx)("h1", { children: "Mapping local conversation stores…" })
		]
	}) : !u.installed || !f ? /* @__PURE__ */ (0, O.jsx)(Pe, {
		home: s,
		probe: u,
		busy: m,
		error: g,
		onInstall: () => void y(),
		onRecheck: v
	}) : i === "operations" ? /* @__PURE__ */ (0, O.jsx)(Re, {
		host: e,
		home: s,
		status: f,
		setStatus: p,
	...
	return /* @__PURE__ */ (0, O.jsxs)("section", {
		...
		children: [
			...
			b,
			/* @__PURE__ */ (0, O.jsxs)("footer", { children: [/* @__PURE__ */ (0, O.jsxs)("span", { children: [
				"Local-only index · ",
				e.backend.kind ?? "unknown",
				" / ",
				e.backend.id ?? "unavailable"
			] }), /* @__PURE__ */ (0, O.jsx)("span", { children: "No conversation content is transmitted by this App" })] })
		]
	});
}
//#endregion
//#region src/styles.css?inline
var Be = ":root{--lightningcss-light: ;--lightningcss-dark:initial;color-scheme:dark}.conversation-search-sidecar{--ink:#f4f1e8;--muted:#999b98;--panel:#171918;--line:#303431;--green:#a6ff4d;--orange:#ff9a62;min-height:100%;color:var(--ink);background:#0d0f0e;font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,sans-serif}.conversation-search-sidecar *{box-sizing:border-box}.conversation-search-sidecar button,.conversation-search-sidecar input,.conversation-search-sidecar select{font:inherit}.css-header{z-index:5;border-bottom:1px solid var(--line);-webkit-backdrop-filter:blur(16px);backdrop-filter:blur(16px);background:#0d0f0ef2;align-items:center;gap:28px;min-height:72px;padding:12px 28px;display:flex;position:sticky;top:0}.css-brand{min-width:260px;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;align-items:center;gap:12px;padding:0;display:flex}.css-brand small,.css-eyebrow{color:var(--green);letter-spacing:.15em;font:700 10px/1.3 ui-monospace,SFMono-Regular,monospace;display:block}.css-brand strong{letter-spacing:-.02em;font-size:16px;display:block}.css-logo{border:1px solid #4b514d;border-radius:9px;place-content:center;gap:3px;width:34px;height:34px;display:grid;transform:rotate(-3deg)}.css-logo i{background:var(--green);width:18px;height:2px;display:block}.css-logo i:nth-child(2){width:12px}.css-logo i:nth-child(3){width:7px}.css-header nav{flex:1;gap:4px;display:flex}.css-header nav button{color:var(--muted);cursor:pointer;background:0 0;border:0;border-radius:7px;padding:9px 12px}.css-header nav button[aria-current=page]{color:var(--ink);background:#222522}.css-runtime{color:var(--muted);align-items:center;gap:7px;font-size:12px;display:flex}.css-runtime span{background:var(--orange);width:7px;height:7px;box-shadow:0 0 12px var(--orange);border-radius:99px}.css-runtime.ready span{background:var(--green);box-shadow:0 0 12px var(--green)}.css-setup,.css-workbench,.css-operations{max-width:1320px;margin:0 auto;padding:42px 34px 80px}.css-hero{border-bottom:1px solid var(--line);grid-template-columns:1fr 360px;align-items:center;gap:30px;min-height:300px;display:grid}.css-hero h1,.css-search-panel h1,.css-operation-hero h1,.css-state h1{letter-spacing:-.055em;max-width:780px;margin:10px 0 14px;font:500 clamp(38px,6vw,76px)/.98 Georgia,serif}.css-hero p,.css-operation-hero p{color:#b4b7b4;max-width:680px;font-size:16px}.css-orbit{border:1px solid #343935;border-radius:50%;place-items:center;width:260px;height:260px;margin:auto;display:grid;position:relative}.css-orbit:before,.css-orbit:after{content:\"\";border:1px solid #252925;border-radius:50%;position:absolute}.css-orbit:before{inset:34px}.css-orbit:after{inset:68px}.css-orbit b{color:var(--green);font:300 72px/1 Georgia}.css-orbit span{background:var(--green);border-radius:50%;width:9px;height:9px;position:absolute}.css-orbit span:first-child{top:20px;left:70px}.css-orbit span:nth-child(2){background:var(--orange);bottom:58px;right:32px}.css-orbit span:nth-child(3){background:#8ab9ff;bottom:75px;left:30px}.css-setup-grid,.css-stats{background:var(--line);border:1px solid var(--line);grid-template-columns:repeat(3,1fr);gap:1px;margin:28px 0;display:grid}.css-setup-grid article,.css-stats article{background:var(--panel);min-width:0;padding:22px}.css-setup-grid article>span{letter-spacing:.08em;background:#2b2e2b;border-radius:3px;padding:3px 7px;font:700 10px ui-monospace,monospace;display:inline-block}.css-setup-grid .ok{color:var(--green)}.css-setup-grid .warn{color:#ffca69}.css-setup-grid .bad{color:#ff796d}.css-setup-grid h2{margin:14px 0 4px;font-size:18px}.css-setup-grid p{color:#c6c8c5;margin:0 0 8px}.css-setup-grid code,.css-location code,.css-danger code,.css-operation-hero code{color:#d8dbd7;overflow-wrap:anywhere;font:11px/1.5 ui-monospace,monospace;display:block}.css-setup-grid small{color:var(--muted)}.css-section,.css-trust,.css-danger{border:1px solid var(--line);background:var(--panel);margin-top:28px;padding:28px}.css-section-title{justify-content:space-between;align-items:flex-start;gap:20px;margin-bottom:18px;display:flex}.css-section-title h1,.css-section-title h2,.css-trust h2{letter-spacing:-.03em;margin:5px 0 0;font-weight:500}.css-locations{gap:8px;display:grid}.css-location{background:#111311;border:1px solid #292d29;padding:16px}.css-location>div{justify-content:space-between;align-items:center;margin-bottom:8px;display:flex}.css-location small{color:var(--muted);text-transform:uppercase;font:10px ui-monospace,monospace}.css-location p{color:var(--muted);margin:7px 0 0}.css-pill{color:#b5b8b4;text-transform:uppercase;background:#282b28;border-radius:20px;padding:3px 7px;font:700 10px ui-monospace,monospace}.css-pill.ready{color:var(--green);background:#20311d}.css-pill.unreadable,.css-pill.unsupported{color:#ff8d7f;background:#392321}.css-pill.empty{color:#ffca69;background:#332c1d}.css-trust{border-left:3px solid var(--green);grid-template-columns:1fr;gap:16px;display:grid}.css-trust p{color:#b5b8b4;max-width:920px}.css-trust code{color:var(--green);font:11px ui-monospace,monospace}.css-privacy{background:#191612;border:1px solid #3b332b;padding:13px 15px}.css-confirm{color:#dadcd8;align-items:center;gap:10px;display:flex}.css-confirm input{accent-color:var(--green)}.css-actions{flex-wrap:wrap;align-items:center;gap:9px;display:flex}.css-button{color:var(--ink);cursor:pointer;background:#222522;border:1px solid #454a45;border-radius:6px;padding:9px 13px}.css-button:hover:not(:disabled){border-color:#727a72}.css-button:disabled{opacity:.45;cursor:not-allowed}.css-button.primary{color:#10120f;background:var(--green);border-color:var(--green);font-weight:700}.css-button.quiet{background:0 0}.css-button.danger{color:#ff8d7f;background:#261817;border-color:#743b36}.css-warning,.css-error,.css-notice{color:#ffd18a;background:#241f15;border:1px solid #6f5832;padding:12px 14px}.css-error{color:#ff9d92;background:#251716;border-color:#6e3732}.css-notice{color:#b9ff81;background:#172015;border-color:#3b5a33}.css-stats{grid-template-columns:repeat(4,1fr);margin:0 0 26px}.css-stats small{color:var(--muted);letter-spacing:.12em;font:10px ui-monospace,monospace}.css-stats strong{margin:8px 0 1px;font:500 28px Georgia,serif;display:block}.css-stats span{color:var(--muted);white-space:nowrap;text-overflow:ellipsis;font-size:11px;display:block;overflow:hidden}.css-search-panel{border:1px solid var(--line);background:linear-gradient(125deg,#191c19,#111311);padding:30px}.css-search-panel h1{font-size:46px}.css-mode{color:var(--green);border:1px solid #385031;border-radius:20px;align-self:flex-start;padding:5px 9px;font:10px ui-monospace,monospace}.css-searchbox{background:#0b0d0c;border:1px solid #4b514b;border-radius:9px;align-items:center;gap:10px;padding:8px 8px 8px 16px;display:flex}.css-searchbox>span{color:var(--green);font-size:24px}.css-searchbox input{min-width:0;color:var(--ink);background:0 0;border:0;outline:0;flex:1;padding:10px 0;font-size:16px}.css-filters{grid-template-columns:repeat(5,1fr);gap:10px;margin-top:12px;display:grid}.css-filters label{color:var(--muted);text-transform:uppercase;font:10px ui-monospace,monospace}.css-filters input,.css-filters select{color:#d8dad7;background:#121412;border:1px solid #333833;border-radius:5px;width:100%;margin-top:5px;padding:8px;display:block}.css-results{border:1px solid var(--line);border-top:0}.css-results-title{color:var(--muted);border-bottom:1px solid var(--line);justify-content:space-between;padding:14px 18px;display:flex}.css-results-title strong{color:var(--ink)}.css-hit{width:100%;color:inherit;border-bottom:1px solid var(--line);background:#111311;padding:20px 22px;display:block}.css-hit:hover{background:#181b18}.css-hit>div{color:var(--muted);text-transform:uppercase;align-items:center;gap:8px;font:10px ui-monospace,monospace;display:flex}.css-hit>div span{border:1px solid #363a36;border-radius:3px;padding:2px 6px}.css-hit .css-role{color:var(--green)}.css-hit time{margin-left:auto}.css-hit h2{margin:10px 0 4px;font-size:16px}.css-hit p{color:#c2c5c1;margin:0 0 10px}.css-conversation-link{color:inherit;text-underline-offset:3px;-webkit-text-decoration:underline #4f7044;text-decoration:underline #4f7044}.css-conversation-link:hover{color:var(--green)}.css-inspect-hit{color:var(--green);cursor:pointer;text-transform:uppercase;background:0 0;border:0;margin:0 0 10px;padding:0;font:11px ui-monospace,monospace;display:inline-block}.css-inspect-hit:hover{text-decoration:underline}.css-hit code{color:#747a74;text-overflow:ellipsis;white-space:nowrap;font:10px ui-monospace,monospace;display:block;overflow:hidden}.css-empty{text-align:center;color:var(--muted);padding:70px}.css-empty>span{color:#4a504a;font:60px Georgia}.css-empty h2{color:var(--ink)}.css-inspector{z-index:10;background:#121412;border-left:1px solid #495049;width:min(580px,90vw);padding:30px;position:fixed;top:72px;bottom:0;right:0;overflow:auto;box-shadow:-20px 0 60px #0008}.css-close{color:var(--muted);cursor:pointer;background:0 0;border:0;font-size:28px;position:absolute;top:15px;right:20px}.css-inspector h2{padding-right:30px;font:500 30px Georgia,serif}.css-inspector dl{gap:8px;display:grid}.css-inspector dl div{border-bottom:1px solid var(--line);grid-template-columns:110px 1fr;gap:12px;padding:9px 0;display:grid}.css-inspector dt{color:var(--muted)}.css-inspector dd{overflow-wrap:anywhere;margin:0}.css-inspector code{font:11px ui-monospace,monospace}.css-inspector pre{white-space:pre-wrap;color:#d5d8d4;border:1px solid var(--line);background:#0b0d0c;max-height:45vh;padding:16px;font:12px/1.6 ui-monospace,monospace;overflow:auto}.css-muted{color:var(--muted);font-size:11px}.css-operation-hero{justify-content:space-between;align-items:flex-end;display:flex}.css-operation-hero h1{font-size:48px}.css-operation-grid{grid-template-columns:repeat(3,1fr);gap:12px;margin:20px 0;display:grid}.css-operation-grid article{border:1px solid var(--line);background:var(--panel);padding:24px;position:relative}.css-operation-grid h2{margin:8px 0}.css-operation-grid p{min-height:105px;color:var(--muted)}.css-operation-grid small{color:#7f857f;margin-top:14px;display:block}.css-step{color:var(--green);font:11px ui-monospace,monospace}.css-danger{border-color:#56302d;justify-content:space-between;align-items:center;gap:30px;display:flex}.css-danger h2{margin:0}.css-danger p{max-width:800px;color:var(--muted)}.css-state{text-align:center;place-content:center;justify-items:center;min-height:65vh;padding:40px;display:grid}.css-state h1{font-size:48px}.css-state p{max-width:700px;color:var(--muted)}.css-loader{gap:5px;margin-bottom:20px;display:flex}.css-loader i{background:var(--green);width:7px;height:28px;animation:.9s infinite alternate css-pulse}.css-loader i:nth-child(2){animation-delay:.15s}.css-loader i:nth-child(3){animation-delay:.3s}@keyframes css-pulse{to{opacity:.35;height:8px}}.conversation-search-sidecar footer{color:#686d68;border-top:1px solid var(--line);text-transform:uppercase;justify-content:space-between;padding:18px 30px;font:10px ui-monospace,monospace;display:flex}@media (width<=900px){.css-header{flex-wrap:wrap}.css-brand{min-width:0}.css-runtime{display:none}.css-hero{grid-template-columns:1fr}.css-orbit{display:none}.css-setup-grid,.css-stats,.css-operation-grid,.css-filters{grid-template-columns:1fr 1fr}.css-operation-grid p{min-height:0}}@media (width<=620px){.css-header{gap:10px;padding:12px 16px}.css-header nav{order:3;width:100%}.css-setup,.css-workbench,.css-operations{padding:28px 16px 60px}.css-setup-grid,.css-stats,.css-operation-grid,.css-filters{grid-template-columns:1fr}.css-searchbox{flex-wrap:wrap}.css-searchbox input{width:calc(100% - 40px)}.css-searchbox button{width:100%}.css-operation-hero,.css-danger,.css-results-title{flex-direction:column;align-items:flex-start}.conversation-search-sidecar footer{flex-direction:column;gap:6px}}";
//#endregion
...
function Ve(e) {
	...
	return e.registerPage("search", ({ container: t, path: n, navigate: r }) => {
		...
		a.dataset.conversationSearchSidecar = "styles", a.textContent = Be;
		let o = document.createElement("div");
		o.dataset.conversationSearchSidecar = "root", t.append(a, o);
		let s = (0, u.createRoot)(o);
		return s.render(/* @__PURE__ */ (0, O.jsx)(ze, {
			host: e,
			path: n,
			navigate: r,
			signal: i.signal
